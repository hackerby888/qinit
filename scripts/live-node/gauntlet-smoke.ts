...
console.log("arithmetic edge cases…");
{
    ...
    [sq, sr, ssum] = (await call(3, "5sint64, 0sint64", "sint64, sint64, sint64")) as bigint[];
...
{
    const nativeFees = (await callFunction(rpc, 1, 1, "", "uint32, uint32, uint32")) as number[];
    ...
    const followUp = (await call(11, "", "uint32, uint32, uint32, uint8")) as number[];
...
console.log("state: Add / HashMap / Array masking / context…");
{
    ...
    expectTrue(BigInt((await call(8, "", "uint64")) as any) >= 1n, "HashMap population >= 1");
    ...
    expectEqual(await call(9, "10uint64", "uint64"), 99n, "Slot(10) == Slot(2) (index masked & 7)");
    ...
    let reward = -1n;
    for (let i = 0; i < 12; i++) {
        const [currentWho, currentReward] = (await call(10, "", "id, sint64")) as [string, bigint];
        if (currentReward === 7n) {
            who = currentWho;
