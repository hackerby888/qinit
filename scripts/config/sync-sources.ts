...
export function loadSourceConfig(root: string): {
    ...
    const qinitRepository = requireValue(qinit.repository, "qinit.repository", repositoryPattern);
    const branch = requireValue(qinit.defaultBranch, "qinit.defaultBranch", branchPattern);
...
export function replaceExactly(source: string, edit: Edit, label: string): string {
...
export function syncSources(root: string, check = false): string[] {
