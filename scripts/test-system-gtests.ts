...
function selection(): Selection {
  ...
  if (selected.length > 1) throw new Error("choose only one of --light, --heavy, or --all");
  return selected[0] ?? "light";
}

const tier = selection();
const coreArg = option("--core") ?? process.env.QINIT_CORE;
if (!coreArg) throw new Error("QINIT_CORE is required (or pass --core <core-lite checkout>)");
...
const filter = new Set((option("--filter") ?? "").split(",").map((name) => name.trim().toUpperCase()).filter(Boolean));
const discovered = systemGtestCorpora(core);
const selected = discovered.filter((entry) =>
  (tier === "all" || entry.tier === tier) && (!filter.size || filter.has(entry.name.toUpperCase()))
);

if (process.argv.includes("--list")) {
  for (const entry of discovered) console.log(`${entry.tier.padEnd(5)} ${entry.name.padEnd(12)} ${entry.corpusPath}`);
  process.exit(0);
}
if (!selected.length) throw new Error(`no ${tier} system gtest corpora matched${filter.size ? `: ${[...filter].join(", ")}` : ""}`);

const sdk = wasiSdkPaths();
if (!sdk) throw new Error("WASI SDK clang/sysroot not found; the real gtest harness requires Clang");
...
for (const [index, entry] of selected.entries()) {
  ...
  try {
    const run = await runCorpus({
      ...
      backend: "local",
      scratch,
      onPhase: (label) => { phase = label; },
    });
    const passed = run.results.filter((result) => result.passed).length;
    const failed = run.results.filter((result) => !result.passed);
    const error = !run.runnerOk
      ? run.buildError ?? "Clang runner build failed"
      : run.results.length === 0
        ? "gtest corpus produced no results"
        : failed.length
          ? failed.map((result) => `${result.name}: ${result.message.replace(/\s+/g, " ").slice(0, 180)}`).join(" | ")
          : undefined;
    rows.push({ name: entry.name, tier: entry.tier, passed, total: run.results.length, seconds: (performance.now() - started) / 1000, error });
    console.log(`  ${error ? "FAIL" : "PASS"} ${passed}/${run.results.length}${error ? ` · ${error}` : ""}\n`);
  } catch (cause) {
    const message = cause instanceof Error ? cause.message : String(cause);
    rows.push({ name: entry.name, tier: entry.tier, passed: 0, total: 0, seconds: (performance.now() - started) / 1000, error: `${phase}: ${message}` });
...
if (passedSuites !== rows.length) process.exitCode = 1;
