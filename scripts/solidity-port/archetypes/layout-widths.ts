...
export const LAYOUT_WIDTH_ARCHETYPES: Archetype[] = [
...

    {
        ...
        caveat: "Documented divergence, pinned deliberately. clang compiles this contract; the TypeScript front end resolves the member's type in the *use* scope instead of at its declaration, follows Inner -> Frame -> Inner forever and dies with `Maximum call stack size exceeded`.",
        axes: ["layout"],
        expectedVerdict: "one-side-rejected",
        divergenceNote:
            "F211 — a nested struct whose name matches a file-scope struct makes the TypeScript analyzer recurse until the stack overflows (Source analysis failed: Maximum call stack size exceeded); clang compiles the same contract.",
        build(axis) {
            const source = emitContract({
                axis,
                name: "LayoutNestedStructNameCollision",
