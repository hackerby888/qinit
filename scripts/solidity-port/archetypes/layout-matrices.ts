    ...
    layoutProbe(
        {
            ...
            caveat: "Documented divergence, pinned deliberately. The TypeScript backend's sizeof parser stops at the comma inside the template argument list; clang compiles both spellings and they agree at 64 bytes. Every other layout archetype in this file aliases the type first for exactly this reason.",
            axes: ["placement"],
            expectedVerdict: "one-side-rejected",
            divergenceNote:
                "F214 — sizeof of a template type with more than one template argument is a parse error in the TypeScript backend (Expected r_paren but got comma in sizeof expr); clang accepts it.",
        },
        () => ({
