...
export const CONTROLFLOW_DISPATCH_ARCHETYPES: Archetype[] = [
...

    {
        ...
        caveat: "Solidity rejects a view function calling a non-view one at compile time, and so does clang: the CALL macro hands the function's context to a procedure and there is no conversion. The TypeScript backend accepts it and *performs the write*, which is what makes this a finding rather than a style difference.",
        axes: ["placement"],
        expectedVerdict: "one-side-rejected",
        divergenceNote:
            "F212 — a PUBLIC_FUNCTION that CALLs a PRIVATE_PROCEDURE: clang refuses it (no viable conversion from QpiContextFunctionCall to QpiContextProcedureCall) while the TypeScript backend compiles it and lets the read-only entry mutate contract state.",
        build(axis) {
            const source = emitContract({
                axis,
                name: "ReadOnlyFunctionCallsPrivateProcedure",
