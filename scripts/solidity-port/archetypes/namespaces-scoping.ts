...
export const NAMESPACE_SCOPING_ARCHETYPES: Archetype[] = [
...

    {
        ...
        caveat: "The contract uses the namespace-qualified spelling next to the global-scope one, so the row proves both `Port::threshold` and `::threshold` parse. The bare `::` spelling used to be refused.",
        axes: ["placement"],
        build(axis) {
            const source = emitContract({
                axis,
