...
//
// The lead archetype here pins a divergence found by reading the backend rather than by sweeping:
// packages/compiler/src/backend/wasm/calls/containers.ts:523 lowers `begin` as
//
//     materializeSelect(context, undefined)
//
// — unconditionally the `any()` selector — and then passes that same buffer for BOTH the ownership
// and the possession parameter of `$lh_assetEnumerate`. `expression.callArguments[1]` and `[2]` are
// never read. So a filtered walk silently enumerates everything. clang compiles core's real iterator
// and honours the filter, so the two backends disagree on a plain wrong answer, not on a refusal.
//
...
const CONTAINERS = "packages/compiler/src/backend/wasm/calls/containers.ts";
    ...
    iterProbe(
        {
            ...
            expectedVerdict: "step-mismatch",
            divergenceNote:
                `The TypeScript backend discards the iterator's filters. ${CONTAINERS}:523 lowers begin() as materializeSelect(context, undefined) — always any() — and passes that one buffer for both the ownership and the possession parameter of $lh_assetEnumerate; callArguments[1] and [2] are never read. clang compiles core's real AssetOwnershipIterator and honours byOwner. With 1000 shares issued and 400 transferred away, the filtered walk should see one holder and 400 shares; the TypeScript build sees both holders and all 1000. The unfiltered control in the same contract agrees on both backends, which is what isolates the cause to the filter rather than to the iteration.`,
        },
        () => ({
    ...
    iterProbe(
        {
            ...
            expectedVerdict: "step-mismatch",
            divergenceNote:
                `Same root cause as AssetOwnershipIteratorOwnerFilterIgnored: ${CONTAINERS}:523 materialises any() for both select parameters regardless of what was written. Here both a byOwner and a byPossessor filter are supplied, so a lowering that honoured either would differ from one that honours neither.`,
        },
        () => ({
            state: "uint64 filteredCount;\nuint64 filteredShares;\nuint64 unfilteredCount;",
    ...
    iterProbe(
        {
            ...
            expectedVerdict: "step-mismatch",
            divergenceNote:
                `${CONTAINERS}:523 again. The id being filtered on never receives any shares, so core's iterator yields nothing and a filter-dropping lowering yields the whole holder set. This is the same defect as the two archetypes above, kept separate because "should be empty" is the cleanest possible statement of it.`,
        },
        () => ({
