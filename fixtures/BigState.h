...
// per-call state copy round-trips the whole thing.
using namespace QPI;
struct CONTRACT_STATE2_TYPE {};
struct CONTRACT_STATE_TYPE : public ContractBase {
    struct StateData { Array<uint8, 64*1024*1024> data; };
    struct Set_input { uint64 v; }; struct Set_output {};
    struct Get_input {}; struct Get_output { uint64 v; };
    PUBLIC_PROCEDURE(Set) { state.mut().data.set(0, (uint8)input.v); state.mut().data.set(60000000, (uint8)(input.v >> 8)); }
    PUBLIC_FUNCTION(Get) { output.v = (uint64)state.get().data.get(0) | ((uint64)state.get().data.get(60000000) << 8); }
