...
test("LiteRpc.get: retries a transient connect failure, then succeeds", async () => {
  let n = 0;
  globalThis.fetch = (async () => { n++; if (n < 3) throw new Error("ECONNREFUSED"); return json({ tick: 5, epoch: 1 }); }) as any;
...
test("LiteRpc.get: exhausts retries -> 'node unreachable'", async () => {
  let n = 0;
  globalThis.fetch = (async () => { n++; throw new Error("boom"); }) as any;
...
test("LiteRpc.get: an HTTP error is a real answer -> NOT retried", async () => {
  let n = 0;
  globalThis.fetch = (async () => { n++; return json({ err: 1 }, 500); }) as any;
