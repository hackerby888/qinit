...
test("uint64 round-trip", async () => {
    const b = await encodeInputFormat("42uint64");
    expect(b.length).toBe(8);
    expect(await decodeAbi(b, "uint64")).toBe(42n);
});

test("smaller scalars", async () => {
    expect(await decodeAbi(await encodeInputFormat("7uint8"), "uint8")).toBe(7);
    expect(await decodeAbi(await encodeInputFormat("258uint16"), "uint16")).toBe(258);
    expect(await decodeAbi(await encodeInputFormat("70000uint32"), "uint32")).toBe(70000);
});

test("signed sint64 negative", async () => {
    expect(await decodeAbi(await encodeInputFormat("-5sint64"), "sint64")).toBe(-5n);
    expect(await decodeAbi(await encodeInputFormat("-1sint32"), "sint32")).toBe(-1);
});

test("uint128 round-trip uses low@0 and high@8 without Number precision loss", async () => {
    const max = (1n << 128n) - 1n;
    const b = await encodeInputFormat(`${max}uint128`);
    expect(b.length).toBe(16);
    expect(hex(b)).toBe("ff".repeat(16));
    expect(await decodeAbi(b, "uint128")).toBe(max);

    const limbs = await encodeInputFormat(`${(2n << 64n) + 3n}uint128`);
    expect(hex(limbs)).toBe("03000000000000000200000000000000");
    expect(await decodeAbi(limbs, "uint128")).toBe((2n << 64n) + 3n);
});

test("uint128 rejects negative and overflowing values", async () => {
    await expect(encodeInputFormat("-1uint128")).rejects.toThrow(/out of range/);
    await expect(encodeInputFormat(`${1n << 128n}uint128`)).rejects.toThrow(/out of range/);
});

test("natural alignment: {uint16, uint32} pads uint32 to offset 4", async () => {
    const b = await encodeInputFormat("5uint16, 7uint32");
    expect(hex(b)).toBe("0500" + "0000" + "07000000"); // val, pad, val
    expect(await decodeAbi(b, "uint16, uint32")).toEqual([5, 7]);
});

test("struct round-trip", async () => {
    const b = await encodeInputFormat("{ 1uint64, 2uint16 }");
    expect(await decodeAbi(b, "{ uint64, uint16 }")).toEqual([1n, 2]);
});

test("array round-trip", async () => {
    const b = await encodeInputFormat("[2; 1uint64, 2uint64]");
    expect(b.length).toBe(16);
    expect(await decodeAbi(b, "[2; uint64]")).toEqual([1n, 2n]);
});

test("nested: array of structs", async () => {
    const b = await encodeInputFormat("[2; { 1uint32, 2uint32 }, { 3uint32, 4uint32 }]");
...
test("id round-trip (32-byte pubkey <-> 60-char identity)", async () => {
    ...
    const id = await decodeAbi(bytes(pub), "id");
    expect(id).toMatch(/^[A-Z]{60}$/);
    expect(hex(await encodeInputFormat(id + "id"))).toBe(pub);
});

test("empty structs encode and decode as one zero byte", async () => {
    expect(await encodeInputFormat("")).toEqual(Uint8Array.of(0));
    expect(await encodeInputFormat("{}")).toEqual(Uint8Array.of(0));
    expect(layoutOf("")).toEqual({ size: 1, align: 1 });
    expect(layoutOf("{}")).toEqual({ size: 1, align: 1 });
    expect(await decodeAbi(Uint8Array.of(0), "")).toEqual([]);
    await expect(decodeAbi(new Uint8Array(0), "")).rejects.toThrow(RangeError);
});

test("nested and arrayed empty structs use one-byte stride", async () => {
    const nested = await encodeInputFormat("{}, 7uint8");
    expect(nested).toEqual(Uint8Array.of(0, 7));
    expect(await decodeAbi(nested, "{}, uint8")).toEqual([[], 7]);
    expect(structFieldOffsets("{}, uint8")).toEqual([
        { off: 0, size: 1 },
        { off: 1, size: 1 },
    ]);

    const array = await encodeInputFormat("[3; {}, {}, {}]");
    expect(array).toEqual(Uint8Array.of(0, 0, 0));
    expect(layoutOf("[3;{}]")).toEqual({ size: 3, align: 1 });
    expect(await decodeAbi(array, "[3;{}]")).toEqual([[], [], []]);
});

test("zero-length arrays remain zero bytes", async () => {
    expect(layoutOf("[0;uint8]")).toEqual({ size: 0, align: 1 });
    expect(await encodeInputFormat("[0;]")).toEqual(new Uint8Array(0));
    expect(await decodeAbi(new Uint8Array(0), "[0;uint8]")).toEqual([]);
});
...
test("m256i (digest) round-trips as hex, not an identity", async () => {
    ...
    const b = await encodeInputFormat(dg + "m256i");
    expect(b.length).toBe(32);
    expect(await decodeAbi(b, "m256i")).toBe(dg);
});

test("id and m256i accept compact zero values", async () => {
    expect(await encodeInputFormat("0id")).toEqual(new Uint8Array(32));
    expect(await encodeInputFormat("0m256i")).toEqual(new Uint8Array(32));
});

test("deep nested: array of structs with an inner array", async () => {
    const b = await encodeInputFormat("[2; { 1uint32, [2; 2uint16, 3uint16] }, { 4uint32, [2; 5uint16, 6uint16] }]");
    expect(await decodeAbi(b, "[2; { uint32, [2; uint16] }]")).toEqual([
        [1, [2, 3]],
        [4, [5, 6]],
    ]);
});
...
test("rejects a malformed id (not 60-char identity nor 64-hex)", async () => {
    ...
    await expect(encodeInputFormat("notavalidlowercaseidentitynotavalidlowercaseidentitynotavaid")).rejects.toThrow(/id must be/);
});

test("rejects a malformed m256i (not 64 hex)", async () => {
    await expect(encodeInputFormat("zzzm256i")).rejects.toThrow(/m256i must be/);
    await expect(encodeInputFormat("1m256i")).rejects.toThrow(/m256i must be/);
    await expect(encodeInputFormat("00112233m256i")).rejects.toThrow(/m256i must be/);
});

test("rejects scalar out of range / bad bit", async () => {
    await expect(encodeInputFormat("300uint8")).rejects.toThrow(/out of range/);
    await expect(encodeInputFormat("-1uint8")).rejects.toThrow(/out of range/);
    await expect(encodeInputFormat("70000uint16")).rejects.toThrow(/out of range/);
    await expect(encodeInputFormat("2bit")).rejects.toThrow(/bit must be/);
});

test("bit round-trips 0/1", async () => {
    expect(await decodeAbi(await encodeInputFormat("1bit"), "bit")).toBe(1);
    expect(await decodeAbi(await encodeInputFormat("0bit"), "bit")).toBe(0);
});

// structFieldOffsets / layoutOf / parseTypeFormat: the alignment math decode-log + state-diff rely on
...
test("parseTypeFormat: empty -> empty struct; unknown type throws", () => {
    expect(parseTypeFormat("")).toEqual({ kind: "struct", fields: [] });
    expect(() => parseTypeFormat("notatype")).toThrow(/unknown type/);
});

test("decodeAbi on a truncated buffer throws (out-of-bounds read)", async () => {
    await expect(decodeAbi(new Uint8Array(4), "uint64")).rejects.toThrow(); // needs 8 bytes
});

test("repeat shorthand: ×N expands to N copies (array, byte-exact + == hand-expanded)", async () => {
    const short = await encodeInputFormat("[4; 9uint32 ×4]");
    const long = await encodeInputFormat("[4; 9uint32, 9uint32, 9uint32, 9uint32]");
    expect(hex(short)).toBe(hex(long));
    expect(hex(short)).toBe("09000000".repeat(4));
    expect(await decodeAbi(short, "[4; uint32]")).toEqual([9, 9, 9, 9]);
});

test("repeat shorthand: x and * variants, spaces optional (9uint32x32 valid)", async () => {
    const ref = await encodeInputFormat("[3; 7uint64, 7uint64, 7uint64]");
    expect(hex(await encodeInputFormat("[3; 7uint64x3]"))).toBe(hex(ref)); // bare x, no space
    expect(hex(await encodeInputFormat("[3; 7uint64 * 3]"))).toBe(hex(ref)); // * with spaces
    expect(hex(await encodeInputFormat("[3; 7uint64×3]"))).toBe(hex(ref)); // × no space
    expect((await encodeInputFormat("[64; 0uint64 ×64]")).length).toBe(64 * 8); // the RANDOM reveal case
});

test("repeat shorthand: struct + top-level reps; non-repeat tokens untouched", async () => {
    const ref = await encodeInputFormat("{1uint32, 2uint32}, {1uint32, 2uint32}, 5uint64");
    expect(hex(await encodeInputFormat("{1uint32, 2uint32} ×2, 5uint64"))).toBe(hex(ref)); // struct repeat
    expect(hex(await encodeInputFormat("5uint64 ×3"))).toBe(hex(await encodeInputFormat("5uint64, 5uint64, 5uint64")));
    expect((await encodeInputFormat("ee".repeat(32) + "id")).length).toBe(32); // 64-hex id (no x): unaffected
});
...
test("zeroInputFormat: builds a schema-matched all-zero sample (scalar/id/array/struct)", async () => {
    ...
    expect(zeroInputFormat("{ uint32, id }")).toBe("0uint32, 0id"); // top-level struct -> implicit field list (no braces, encodeInputFormat-consistent)
...
test("zeroInputFormat: the sample is valid input — encodes to exactly the layout size", async () => {
    for (const fmt of ["uint64", "uint128", "[64; uint64], id", "{ uint32, id }", "uint16, uint32", "m256i", "[3; { uint8, uint64 }]"]) {
        ...
        const b = await encodeInputFormat(sample);
...
test("parseTypeFormat rejects a malformed array instead of looping or yielding NaN", () => {
    expect(() => parseTypeFormat("[2 uint8]")).toThrow(/array needs a ';'/); // used to recurse until the stack blew
    expect(() => parseTypeFormat("[abc; uint8]")).toThrow(/array count 'abc' must be a non-negative integer/);
    ...
    expect(() => parseTypeFormat("[2;uint8")).toThrow(/array is missing its closing ']'/);
});

test("parseTypeFormat names an unterminated struct and a stray delimiter", () => {
    expect(() => parseTypeFormat("{uint64")).toThrow(/struct is missing its closing '}'/);
    expect(() => parseTypeFormat("}")).toThrow(/expected a type at position 0/);
    expect(parseTypeFormat("{ uint8, }")).toEqual({ kind: "struct", fields: [parseTypeFormat("uint8")] }); // a trailing comma stays legal
    expect(() => parseTypeFormat("notatype")).toThrow(/unknown type/); // still the message for a real typo
});

test("encodeInputFormat rejects an array whose declared count does not match its values", async () => {
    await expect(encodeInputFormat("[5; 1uint8]")).rejects.toThrow(/array of 5 needs 5 values, got 1/);
    await expect(encodeInputFormat("[0; 1uint8, 2uint8]")).rejects.toThrow(/array of 0 needs 0 values, got 2/);
    await expect(encodeInputFormat("[2;{}]")).rejects.toThrow(/array of 2 needs 2 values, got 1/);
});
...
test("the value dialect needs the bracket that closes what it opened", async () => {
    ...
    expect(await encodeInputFormat("[2; 1uint8, 2uint8]")).toEqual(new Uint8Array([1, 2]));
    for (const bad of ["[2; 1uint8, 2uint88", "[2; 1uint8, 2uint8}", "[2; 1uint8, 2uint8)", "[2; 1uint8, 2uint8]junk"]) {
        await expect(encodeInputFormat(bad)).rejects.toThrow(`array value is missing its closing ']': '${bad}'`);
    }
    for (const bad of ["{ 1uint8, 2uint8x", "{ 1uint8, 2uint8]", "{ 1uint8, 2uint8 }junk"]) {
        await expect(encodeInputFormat(bad)).rejects.toThrow(`struct value is missing its closing '}': '${bad}'`);
    }
    // The ×N shorthand still reaches encodeToken with the closer intact, because expandReps ran first.
    expect((await encodeInputFormat("[1; 1uint64]x2")).length).toBe(16);
});
...
test.each(BOUNDS)("%s encodes and reads back both of its limits", async (type, low, lowHex, high, highHex) => {
    for (const [value, expected] of [
        ...
        const bytes = await encodeInputFormat(`${value}${type}`);
        expect({ value, hex: hex(bytes) }).toEqual({ value, hex: expected });
        expect(String(await decodeAbi(bytes, type))).toBe(value);
    }
});

test.each(BOUNDS)("%s rejects the value just past each of its limits", async (type, low, _lowHex, high) => {
    await expect(encodeInputFormat(`${BigInt(low) - 1n}${type}`)).rejects.toThrow(`${type} out of range`);
    await expect(encodeInputFormat(`${BigInt(high) + 1n}${type}`)).rejects.toThrow(`${type} out of range`);
});

test("bit takes 0 and 1 and nothing either side", async () => {
    expect(hex(await encodeInputFormat("0bit"))).toBe("00");
    expect(hex(await encodeInputFormat("1bit"))).toBe("01");
    await expect(encodeInputFormat("2bit")).rejects.toThrow("bit must be 0 or 1, got 2");
    await expect(encodeInputFormat("-1bit")).rejects.toThrow("bit must be 0 or 1, got -1");
});
...
test.each(MALFORMED)("both dialects reject %s", async (_case, typeFormat, valueFormat) => {
    ...
    await expect(encodeInputFormat(valueFormat)).rejects.toThrow();
});
...
test.each(WELL_FORMED)("both dialects accept %s", async (_case, typeFormat, valueFormat) => {
    ...
    expect(await encodeInputFormat(valueFormat)).toHaveLength(layout.size);
});

test("a missing separator is an error, not a shorter layout", () => {
    // parseTypeFormat used to keep parseType's node and drop the index saying where it stopped, so a dropped ',' read back as a layout with fewer fields.
...
test("the value dialect rejects the same junk array counts the type dialect does", async () => {
    ...
    for (const bad of ["2uint64", "2.9", "0x10", "abc", "-1", " "]) {
        await expect(encodeInputFormat(`[${bad}; 1uint64, 2uint64]`)).rejects.toThrow(`array count '${bad.trim()}' must be a non-negative integer`);
        expect(() => parseTypeFormat(`[${bad};uint64]`)).toThrow(`array count '${bad.trim()}' must be a non-negative integer`);
    }
    expect((await encodeInputFormat("[2; 1uint64, 2uint64]")).length).toBe(16);
});

test("the count check counts values after ×N expansion, so the existing shorthands still pass", async () => {
    expect(await encodeInputFormat("[0;]")).toEqual(new Uint8Array(0));
    expect((await encodeInputFormat("[3; {}, {}, {}]")).length).toBe(3);
    expect((await encodeInputFormat("[64; 0uint64 ×64]")).length).toBe(512);
    expect((await encodeInputFormat("[2; 1uint128 ×2]")).length).toBe(32);
    expect((await encodeInputFormat("[4; 9uint32 ×4]")).length).toBe(16);
});
...
test("sint128 parses, sizes, and round-trips as a signed 128-bit value", async () => {
    expect(parseTypeFormat("sint128")).toEqual({ kind: "sint128" });
    expect(layoutOf("sint128")).toEqual({ size: 16, align: 8 });

    expect(await decodeAbi(new Uint8Array(16).fill(0xff), "sint128")).toBe(-1n);
    expect(hex(await encodeInputFormat("-1sint128"))).toBe("ff".repeat(16));

    const min = -(1n << 127n);
    const max = (1n << 127n) - 1n;
    expect(await decodeAbi(await encodeInputFormat(`${min}sint128`), "sint128")).toBe(min);
    expect(await decodeAbi(await encodeInputFormat(`${max}sint128`), "sint128")).toBe(max);
});

test("sint128 rejects values outside the signed range and non-integers", async () => {
    await expect(encodeInputFormat(`${1n << 127n}sint128`)).rejects.toThrow(/out of range/);
    await expect(encodeInputFormat(`${-(1n << 127n) - 1n}sint128`)).rejects.toThrow(/out of range/);
    await expect(encodeInputFormat("1.5sint128")).rejects.toThrow(/sint128 must be an integer/);
});
...
test("sint128 aligns and pads like uint128 inside a struct", async () => {
    ...
    expect((await encodeInputFormat("[2; 0sint128 ×2]")).length).toBe(32);
});
...
test("the string and typed decoders agree on the same bytes", async () => {
    for (const [label, raw, size, format] of CROSS_PATH_ROWS) {
        ...
        expect(await decodeAbi(bytes, format)).toEqual(await decodeAbiValue(bytes, type));
    }
});
...
test("a nested value format encodes and decodes byte-for-byte through three levels", async () => {
    const encoded = await encodeInputFormat("{ 1uint8, [2; {2uint8, 3uint64}, {4uint8, 5uint64}] }");
    expect(encoded.length).toBe(layoutOf("{ uint8, [2;{uint8,uint64}] }").size); // 40: the array aligns to 8 first
    expect(await decodeAbi(encoded, "{ uint8, [2;{uint8,uint64}] }")).toEqual([
        1,
        [
            [2, 3n],
            [4, 5n],
    ...
    const matrix = await encodeInputFormat("[2; [2; {1uint8, 2uint64}, {3uint8, 4uint64}], [2; {5uint8, 6uint64}, {7uint8, 8uint64}]]");
    expect(matrix.length).toBe(layoutOf("[2;[2;{uint8,uint64}]]").size);
    expect(await decodeAbi(matrix, "[2;[2;{uint8,uint64}]]")).toEqual([
        [
            [1, 2n],
            [3, 4n],
        ],
...
test("the ×N shorthand expands inside a nested struct inside an array", async () => {
    const short = await encodeInputFormat("[2; { 1uint8, [3; 2uint16 ×3] } ×2]");
    const long = await encodeInputFormat("[2; { 1uint8, [3; 2uint16, 2uint16, 2uint16] }, { 1uint8, [3; 2uint16, 2uint16, 2uint16] }]");
    ...
    expect(await decodeAbi(short, "[2;{ uint8, [3;uint16] }]")).toEqual([
        [1, [2, 2, 2]],
        [1, [2, 2, 2]],
    ]);
});
...
test("a ×0 repeat contributes no values and satisfies a zero-length array", async () => {
    expect(await encodeInputFormat("[0; 1uint8 ×0]")).toEqual(new Uint8Array(0));
    expect(await encodeInputFormat("5uint64 ×0")).toEqual(new Uint8Array(0));
    await expect(encodeInputFormat("[1; 1uint8 ×0]")).rejects.toThrow(/array of 1 needs 1 values, got 0/);
});

// A self-consistent value format can still be the wrong shape for its entry, and encodeInputFormat cannot see that — these pin the size cross-check the callers run.
test("assertInputSize passes an input whose encoding matches the entry", async () => {
    const type = validated(st(arr(u16, 4), u32));
    expect(type.size).toBe(12);
    const input = await encodeInputFormat("[4; 1uint16, 2uint16, 3uint16, 4uint16], 9uint32");
    expect(() => assertInputSize(type, input, "proc 1/2")).not.toThrow();
});

test("assertInputSize rejects a self-consistent input of the wrong shape", async () => {
    const type = validated(st(arr(u16, 4), u32));
    ...
    const short = await encodeInputFormat("[2; 1uint16, 2uint16], 9uint32");
    expect(short.length).toBe(8);
    expect(() => assertInputSize(type, short, "proc 1/2")).toThrow("encodes to 8 bytes, proc 1/2 wants 12 ({ [4;uint16], uint32 })");
});

test("assertInputSize rejects an input that drops the array brackets entirely", async () => {
    const type = validated(st(arr(u16, 4), u32));
    const flat = await encodeInputFormat("1uint16, 2uint16, 9uint32");
    expect(() => assertInputSize(type, flat, "proc 1/2")).toThrow(/encodes to 8 bytes, proc 1\/2 wants 12/);
});

test("assertInputSize accepts the empty input an entry with no arguments encodes to", async () => {
    const type = validated(st());
    const empty = await encodeInputFormat("");
    expect(empty.length).toBe(type.size);
    expect(() => assertInputSize(type, empty, "proc 1/1")).not.toThrow();
});

test("assertInputSize names an over-long input against an entry that takes none", async () => {
    const type = validated(st());
    const extra = await encodeInputFormat("1uint64");
    expect(() => assertInputSize(type, extra, "proc 1/1")).toThrow(/encodes to 8 bytes, proc 1\/1 wants 1/);
});
...
test("the schema road lays the spelled tokens out exactly like --args does", async () => {
    ...
    expect(await encodeInputFormatAs(MIRROR, spelled)).toEqual(typed);
    expect(await encodeInputFormatAs(MIRROR, `{${spelled}}`)).toEqual(typed);
    expect(await encodeInputFormatAs(MIRROR, spelled)).toEqual(await encodeInputFormat(spelled));
});
...
test("a token of the wrong width or in the wrong order is refused by field name", async () => {
    ...
    expect(await encodeInputFormatAs(signed, "-1sint8, -2sint16, -3sint32")).toEqual(await encodeInputFormat("-1sint8, -2sint16, -3sint32"));
    await expect(encodeInputFormatAs(signed, "-2sint16, -1sint8, -3sint32")).rejects.toThrow("input.s8 is sint8, got '-2sint16'");

    const pair = named(["a", u8], ["b", u64]);
    await expect(encodeInputFormatAs(pair, "1uint64, 2uint64")).rejects.toThrow("input.a is uint8, got '1uint64'");
    await expect(encodeInputFormatAs(pair, "1uint8")).rejects.toThrow("input has 2 field(s), got 1 value(s)");
    await expect(encodeInputFormatAs(pair, "")).rejects.toThrow("input has 2 field(s), got 0 value(s)");
    await expect(encodeInputFormatAs(pair, "1uint8, {2uint64}")).rejects.toThrow("input.b is uint64, got '{2uint64}'");
    await expect(encodeInputFormatAs(named(["n", arr(u64, 2)]), "[3; 1uint64, 2uint64, 3uint64]")).rejects.toThrow("input.n expects 2 elements, got 3");
    await expect(encodeInputFormatAs(named(["n", arr(u64, 2)]), "1uint64, 2uint64")).rejects.toThrow("input has 1 field(s), got 2 value(s)");
    await expect(encodeInputFormatAs(pair, "1uint8, 300uint64x1")).resolves.toBeInstanceOf(Uint8Array);
    await expect(encodeInputFormatAs(pair, "256uint8, 1uint64")).rejects.toThrow("uint8 out of range: 256");
});
...
test("the schema road keeps the repeat shorthand, zero ids, and physical bit-array words", async () => {
    ...
    const bytes = await encodeInputFormatAs(four, `[4; 9uint32x4], 0id, ${words}`);

    expect(bytes).toEqual(await encodeInputFormat(`[4; 9uint32x4], 0id, ${words}`));
    expect(bytes.subarray(16, 48)).toEqual(new Uint8Array(32));
    await expect(encodeInputFormatAs(four, "[4; 9uint32x4], 0id, [1; 1uint64]")).rejects.toThrow("input.bits encodes to 8 bytes, [2;uint64] wants 16");
});
