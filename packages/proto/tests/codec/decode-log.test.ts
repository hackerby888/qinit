...
const LOGGER = {
  ...
  fields: ["_contractIndex", "_type", "value"],
};
...
test("decodeLog resolves the _type discriminator to its enum name", async () => {
  const cat = [
    { name: "LogMsg", fmt: "uint32, uint32, uint64", fields: ["_contractIndex", "_type", "value"] },
  ];
...
test("a malformed fmt in the catalog is skipped, a valid sibling still matches", async () => {
  const cat = [
    { name: "bad", fmt: "struct nope", fields: [] },
    { name: "good", fmt: "uint64", fields: ["v"] },
  ];
