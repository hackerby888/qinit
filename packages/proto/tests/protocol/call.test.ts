...
test("resolveDeploymentSlot rejects explicit slots outside the dynamic window", async () => {
  ...
  await expect(resolveDeploymentSlot(rpc, "Counter", 28)).rejects.toThrow(
    "slot 28 is outside dynamic range 29..32",
  );
  await expect(resolveDeploymentSlot(rpc, "Counter", 33)).rejects.toThrow(
    "slot 33 is outside dynamic range 29..32",
  );
...
test("resolveDeploymentSlot protects explicit slots and reuses the same name", async () => {
  const occupied = rpcWithRegistry([
    contract(29, true, "Other"),
    contract(30, true, "Counter"),
  ]);

  await expect(resolveDeploymentSlot(occupied, "Counter", 29)).rejects.toThrow(
    "slot 29 is occupied by 'Other', not 'Counter'",
  );
  await expect(resolveDeploymentSlot(occupied, "Counter", 31)).rejects.toThrow(
    "'Counter' is already deployed at slot 30, not requested slot 31",
  );
...
test("resolveDeploymentSlot ignores same-name and free entries outside the dynamic window", async () => {
  const rpc = rpcWithRegistry([
    contract(1, true, "Counter"),
    contract(28, false),
    contract(29, false),
    contract(30, true, "Other"),
  ]);
