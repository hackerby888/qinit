...
export function moveLocalToWithLocalsEdits(source: string, nameOffset: number, nameLength: number): SourceEdit[] | null {
  ...
  let j = nameOffset + nameLength, eq = -1;
  ...
  if (structM && structM.index !== undefined) {
    ...
    const indent = (source.slice(source.lastIndexOf("\n", fn.macroStart - 1) + 1, fn.macroStart).match(/^\s*/) ?? [""])[0];
    edits.push({ start: fn.macroStart, end: fn.macroStart, newText: `struct ${fn.name}_locals { ${field} };\n${indent}` });
  }
  ...
  if (eq >= 0) {
    ...
    const alone = source.slice(lineStart, typeStart).trim() === "" && source.slice(semi + 1, lineEnd).trim() === "";
    edits.push(alone ? { start: lineStart, end: lineEnd, newText: "" } : { start: typeStart, end: semi + 1, newText: "" });
  }
