...
import { dirname, join, resolve, relative as relativePath } from "node:path";
...
/** carries the completion settings and names its database, so a `compile_commands.json` in a subfolder cannot shadow it. Relative, so a clone keeps it. */
function ensureClangdConfig(workspaceRoot: string, databaseDir: string, sourcePath: string): { path: string; configured: boolean; rewritten: boolean } {
    const path = join(workspaceRoot, ".clangd");
    const relative = forwardSlashes(relativePath(workspaceRoot, databaseDir)) || ".";
    const body = [
        CLANGD_MARKER,
        "CompileFlags:",
        `  CompilationDatabase: ${JSON.stringify(relative)}`,
        "Completion:",
        "  AllScopes: No",
        "  HeaderInsertion: Never",
        "",
    ].join("\n");
    const rewritten = removeShadowingConfigs(workspaceRoot, sourcePath);

    if (existsSync(path)) {
        const current = readFileSync(path, "utf8");
        if (!current.startsWith(CLANGD_MARKER)) {
            // no marker: the developer's own file, not ours to rewrite.
            return { path, configured: relative === "." || current.includes(relative), rewritten };
        }
        if (current === body) {
            return { path, configured: true, rewritten };
        }
    }
...
function removeShadowingConfigs(workspaceRoot: string, sourcePath: string): boolean {
    ...
    for (let dir = dirname(resolve(sourcePath)); dir !== root && dir.startsWith(root); dir = dirname(dir)) {
        const nested = join(dir, ".clangd");
        if (existsSync(nested) && readFileSync(nested, "utf8").startsWith(CLANGD_MARKER)) {
            rmSync(nested);
            removed = true;
        }
    }
...
export function generateClangdConfig(o: ClangdInputs): ClangdConfig {
    ...
    const clangd = ensureClangdConfig(o.workspaceRoot, dbDir, details.contractFile);
...
export function generateTestClangdConfig(o: ClangdInputs & { testPath: string }): TestClangdConfig {
    ...
    const clangd = ensureClangdConfig(o.workspaceRoot, dbDir, testFile);
    ensureEditorSettings(o.workspaceRoot);
