...
test("project and contract discovery work without qinit.json", () => {
    ...
    try {
        ...
        expect(configuredContractIdentity(join(root, "contracts", "Counter.h"))).toEqual({
            name: "Counter",
            slot: 42,
        });
        expect(configuredContractIdentity(join(root, "contracts", "Plain.h"))).toEqual({});
    } finally {
...
test("config saves select only open contracts from that project", () => {
    ...
    try {
        ...
        const documents = [
            ...
            document(join(other, "Other.h"), "struct Other : public ContractBase {};"),
        ];

        expect(projectContractDocuments(join(root, "qinit.json"), documents)).toEqual([contract]);
    } finally {
        rmSync(root, { recursive: true, force: true });
