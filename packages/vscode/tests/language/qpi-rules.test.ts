...
test("allowed forms do not trigger", () => {
    ...
    expect(rulesOf("uint64 p = a * b;")).not.toContain("qpi/no-division");
...
// Core wraps contract includes in `#define CONTRACT_STATE_TYPE <Name>`, so a struct named by the macro has no name outside core — where callers still need one.
test("the contract struct has to carry its own name, not core's macro", () => {
    ...
    // Only the contract struct: a plain struct carrying the name is core's own second state type, declared without a base, and is left alone.
