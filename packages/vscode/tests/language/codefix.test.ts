...
function qpiCodes(source: string): string[] {
    return analyzeContract({ source })
        .diagnostics.filter((item) => item.origin === SourceAnalysisOrigin.QPI)
        .map((item) => item.code);
}
...
test("rewrites a member array declaration to Array<T, N>", () => {
    expect(applyFix("  uint64 cells[8];", "qpi/no-brackets")).toBe("  Array<uint64, 8> cells;");
...
test("keeps a local initializer", () => {
    const source = wrap("        uint64 sum = input.amount;\n        state.mut().total = sum;");
    const output = applyFix(source, "qpi/stack-local")!;
