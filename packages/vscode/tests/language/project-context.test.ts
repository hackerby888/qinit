...
test.if(hasCore)(
  "Proxy resolves Counter from contracts and configures clangd without a node",
  () => {
    ...
    try {
      ...
      expect(clangd.args).toContain(
        `--sysroot=${bundledSysroot.replace(/\\/g, "/")}`,
      );
