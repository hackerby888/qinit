...
test.if(hasFixture)("generateClangdConfig: prefix carries the wrapper preamble; DB parses the contract with it", () => {
    ...
    try {
        ...
        // the database sits at the root, and `.clangd` still names it so a subfolder's database cannot shadow it.
        expect(r.dbPath).toBe(join(ws, "compile_commands.json"));
        const dotClangd = readFileSync(join(ws, ".clangd"), "utf8");
        expect(dotClangd).toContain('CompilationDatabase: "."');
...
// A subfolder opened as its own workspace earlier leaves a generated `.clangd` behind that points its subtree at a database missing the new files.
test("a generated .clangd under the workspace root is removed; a developer's own one stays", () => {
    const ws = mkdtempSync(join(tmpdir(), "qpi-nested-"));
    try {
        const contractsDir = join(ws, "Counter", "contracts");
        mkdirSync(contractsDir, { recursive: true });
        const contract = join(contractsDir, "Counter.h");
        writeFileSync(contract, "using namespace QPI;\nstruct Counter : public ContractBase { struct StateData { uint64 x; }; };\n");
        const stale = join(ws, "Counter", ".clangd");
        writeFileSync(stale, '# Generated by the Qubic QPI extension.\nCompileFlags:\n  CompilationDatabase: "/old/storage"\n');
        const owned = join(contractsDir, ".clangd");
        writeFileSync(owned, "# mine\n");

        const first = generateClangdConfig({ contractPath: contract, corePath: "/fake/core", workspaceRoot: ws });
        expect(existsSync(stale)).toBe(false);
        expect(readFileSync(owned, "utf8")).toBe("# mine\n");
        expect(first.restartRequired).toBe(true);

        const second = generateClangdConfig({ contractPath: contract, corePath: "/fake/core", workspaceRoot: ws });
...
test.if(hasFixture)("a root the developer already occupies is worked around, never clobbered", () => {
    ...
    // A free root: the database lands there and .clangd names it, so nothing nearer a file can shadow it.
    const pristine = run(() => {});
    expect(pristine.relocated).toBe(false);
    expect(pristine.result.clangdConfigured).toBe(true);
    expect(pristine.dotClangd).toContain('CompilationDatabase: "."');
