...
const required = [
    ...
    join(resources, "core-headers", "wasi-sdk", "share", "wasi-sysroot", "include", "wasm32-wasi", "c++", "v1", "cstdint"),
...
const repositories = JSON.parse(readFileSync(join(repositoryRoot, "config", "repositories.json"), "utf8")) as {
    coreLite: { repository: string };
};
const snapshotManifest = JSON.parse(readFileSync(join(repositoryRoot, "packages", "compiler", "core-snapshot.json"), "utf8")) as { coreCommit: string };
...
if (revision.exitCode !== 0 || actualCommit !== snapshotManifest.coreCommit) {
    throw new Error(`core commit ${actualCommit || "unknown"} does not match ` + snapshotManifest.coreCommit);
}
