...
const CodeLine = ({ ln }: { ln: string }) => {
  ...
  return (
    <Text>
      {parts.map((p, j) => (
        <Text key={j} color={/^[A-Za-z0-9_]+$/.test(p) ? tokenColor(p) : undefined}>
          {p}
        </Text>
      ))}
...
const Fmt = ({ k, ex, note }: { k: string; ex: string; note?: string }) => (
  <Text>
    {"  "}
    <Text color={theme.info}>{k.padEnd(9)}</Text> <CodeLine ln={ex} />
...
export function Cheat() {
  ...
  return (
    <Box flexDirection="column">
      <Box marginBottom={1}>
        <Grad text="qinit cheat-sheet" />
        <Text dimColor> — set up · deploy · call</Text>
      </Box>

      <Panel title="1 · setup" color={theme.brand}>
        <Text>
          <C>curl -fsSL .../install.sh | sh</C> <D>install qinit</D>
        </Text>
        <Text>
          <C>qinit node run</C> <D>fetch node + headers, run a local testnet node</D>
        </Text>
        <Text>
