...
type S = { phase: "run" | "preview" | "done" | "err"; bins?: string[]; cacheTotal?: number; freed?: number; killed?: boolean; removed?: string[]; err?: string };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!go) { setS({ phase: "preview", bins, cacheTotal: cacheInfo().total }); return; }
    ...
    <Box flexDirection="column">
      ...
      {s.phase === "preview" && (
        <Box flexDirection="column">
          ...
          <Box marginLeft={2}><KV rows={[
            ...s.bins!.map((b) => ["binary", b] as [string, string]),
            ["cache", keepCache ? "(kept — --keep-cache)" : `${cacheInfo().root}  ${human(s.cacheTotal!)}`],
          ]} /></Box>
          <Box marginTop={1}><Text dimColor>re-run to confirm: </Text><Text bold color={theme.accent}>qinit uninstall --yes</Text></Box>
        </Box>
      )}
      {s.phase === "done" && (
        <Box flexDirection="column">
          <Status ok={true} label={`uninstalled${keepCache ? " (cache kept)" : ` · freed ${human(s.freed!)}`}`} />
          {s.removed!.length ? <Box marginLeft={2} flexDirection="column">{s.removed!.map((b, i) => <Text key={i} dimColor>removed {b}</Text>)}</Box> : <Text dimColor>(no binary path found to remove)</Text>}
          {s.killed ? <Text dimColor>(stopped a running node first)</Text> : null}
          <Box marginTop={1}><Text dimColor>thanks for using qinit · reinstall: </Text><Text bold color={theme.accent}>curl -fsSL …/install.sh | sh</Text></Box>
        </Box>
      )}
    </Box>
  );
