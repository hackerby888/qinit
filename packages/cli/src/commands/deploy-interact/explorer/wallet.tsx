...
export function WalletView({
    ...
    void (async () => {
      ...
      if (poolResult.status === "fulfilled") {
        ...
        const entries = await Promise.all(
          seeds.map(async (seed) => [(await deriveIdentity(seed)).identity, seed] as const),
        );
        if (!alive) {
          return;
        }
  ...
  const balanceHint =
    balance != null ? `${fmtAmount(balance.toString())} qu` : undefined;
  const remaining =
          ...
          hint={
            <HintLine
              ...
              extra={
                from.status === "ok"
                  ? balanceHint
                  : shapeHint(classifyWalletInput(fromInput.trim()), fromInput.trim())
              }
            />
          }
        />
