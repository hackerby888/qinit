...
export function Debug({ commandArgs }: { commandArgs: CommandArguments }) {
    ...
    useEffect(() => {
        ...
        for (const tick of timestampTicks) {
            tickTimeAttempts.current.set(tick, (tickTimeAttempts.current.get(tick) ?? 0) + 1);
        }
        Promise.all(
...
function Detail({
    ...
    const fixedRows =
        ...
        (v?.cheats.reduce((count, cheat) => count + (cheat.blocks ? 2 : 1), 0) ?? 0) + // a print, and a block's one-line count
        (v?.logs.length ?? 0) +
        e.hostCalls.length +
        (e.trap ? 1 : 0);
