  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const env = { ...process.env, QINIT_RPC: activeRpc, QINIT_SEED: seed, QINIT_CONTRACT: String(dep.slot) };
        ...
        if (ownNode && o.keep === undefined) await killNode();
        engineSrv?.stop();
        setS({ phase: "done", lines: L, ok, output, rows: [["contract", `${name} @ ${dep.slot}`], ["rpc", activeRpc], ["node", engineSrv ? "in-process engine" : (ownNode ? (o.keep === undefined ? "ephemeral (stopped)" : "ephemeral (kept)") : "reused")]] });
      } catch (e: any) {
