...
function parseAssetName(value: string): string {
  if (!/^[A-Z][A-Z0-9]{0,6}$/.test(value)) {
    throw new Error(
      "asset must be 1–7 uppercase letters or digits, starting with a letter",
    );
  }
...
export function Integrate({ commandArgs }: { commandArgs: CommandArguments }) {
  ...
  const promptField = state.phase === "prompt"
    ? state.fields[state.index]
    : undefined;
  const promptLabel = promptField === "assetName"
    ? "asset name"
