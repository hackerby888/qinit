...
export const META: Record<string, CommandMeta> = {
  ...
  gtest: { group: "deploy & interact", summary: "run a contract's core-lite contract_testing.h gtest on a fresh isolated virtual node", usage: "[<test.cpp>]", flags: [["--contract <file.h>", "contract under test (default: qinit.json)"], ["--name <Name>", "contract name override"], ["--state-type <T>", "C++ contract struct type, if it differs from the name"], ["--filter <pat>", "test name substring filter"], ["--new", "(re)scaffold tests/<Name>.test.cpp from the IDL"], ["--core <path>", "core-lite headers (with test/contract_testing.h)"], ["--local", "build the contract-under-test with the in-process TS compiler"], ["--shared-mem", "run the contract in shared-memory mode (huge-state suites)"], ["--corpus <NAME>", "run a built-in system contract's gtest by name (core-lite contract_<x>.cpp)"]] },
