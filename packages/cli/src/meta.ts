...
export const META: Record<string, CommandMeta> = {
  ...
  gtest: { group: "deploy & interact", summary: "compile a C++ gtest INTO the contract wasm and run it on a fresh isolated virtual node (no native toolchain)", usage: "[<test.cpp>]", flags: [["--contract <file.h>", "contract under test (default: qinit.json)"], ["--name <Name>", "contract name override"], ["--filter <pat>", "test name substring filter"], ["--new", "(re)scaffold tests/<Name>.test.cpp from the IDL"], ["--core <path>", "core-lite headers (with extensions/lite_test.h)"]] },
