...
export async function decodeColumns(rpc: StateReader, idx: number, fields: StateField[]): Promise<ColView[]> {
  const out: ColView[] = [];
...
export async function readState(rpc: StateReader, idx: number, source: string, name: string): Promise<StateDump> {
  const idl = extractIdl(source, name);
  ...
  for (const f of fields) {
    ...
    try {
      ...
      scalars.push({ name: f.name, value: typeof dv === "object" && dv !== null ? jstr(dv) : String(dv) });  // bare scalar unquoted; struct/array as json
    } catch { scalars.push({ name: f.name, value: "(read failed)" }); }
  }
  return { fields: scalars, cols: await decodeColumns(rpc, idx, fields) };
}
