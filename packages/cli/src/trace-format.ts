...
export function stateFieldsOf(idl: {
  state?: { name: string; type: string; container?: Container }[];
}): StateField[] {
  let acc = 0;
  ...
  for (const f of idl.state ?? []) {
    ...
    acc = roundUp(acc, L.align);
    out.push({ name: f.name, off: acc, size: L.size, type: f.type, container: f.container });
    acc += L.size;
  }
  return out;
}
...
export function enumMap(idl: {
  enums?: { name: string; members: Record<string, string> }[];
...
export async function decodeColumns(
  ...
  for (const f of fields) {
    ...
    try {
      ...
      const ents =
        c.kind === "hashmap"
          ? (await decodeHashMap(buf, c.keyFmt, c.valFmt!, c.capacity)).map(
              (x) => `${keyLabel(x.key)} = ${fmtVal(x.value, full)}`,
            )
          : c.kind === "collection"
            ? (await decodeCollection(buf, c.valFmt!, c.capacity)).map(
    ...
    try {
      ...
      if (e.logs?.length)
        logs = await Promise.all(
          e.logs.map((l) => decodeLog(l.type, l.size, l.hex, idl.logStructs ?? [], em)),
        );
    } catch {}
  }
...
export async function readState(
  ...
  for (const f of fields) {
    ...
    try {
      ...
      if (am && f.size > CAP) {
        const n = Number(am[1]);
        const elem = am[2].trim();
        const el = layoutOf(elem);
        const stride = Math.max(1, roundUp(el.size, el.align));
