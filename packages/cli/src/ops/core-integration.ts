...
import { parseContractDef, scanCallees } from "@qinit/build/intercontract";
...
): Required<Pick<CoreIntegrationOptions, "assetName" | "constructionEpoch" | "destructionEpoch">> {
    return {
        assetName: options.assetName ?? existing!.assetName,
        constructionEpoch: options.constructionEpoch ?? existing!.constructionEpoch,
...
function planMutations(options: {
    ...
    return {
        mutations,
        contractIndex,
        testPath: testSource || existsSync(testDestination) ? testDestination : undefined,
        warnings,
    };
}
