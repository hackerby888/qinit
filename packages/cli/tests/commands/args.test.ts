...
import { initOutput, nearest, output, parseArgs, parseCommandInvocation } from "../../src/args";
...
test("parseCommandInvocation gets definitions from command metadata", () => {
    ...
    expect(invocation.commandArgs.getAll("callee")).toEqual(["A=a.h@1", "B=b.h@2"]);
});
...
test("parseCommandInvocation resolves and strips a known first subcommand", () => {
    const invocation = parseCommandInvocation("node", ["run", "--restart", "--rpc", "http://x"]);
