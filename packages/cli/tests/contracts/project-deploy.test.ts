...
test.skipIf(!haveCore || !canListen)(
    ...
    async () => {
        ...
        copyFileSync(resolve(import.meta.dir, "../../../../fixtures/Counter.h"), counterPath);
        copyFileSync(resolve(import.meta.dir, "../../../../fixtures/Proxy.h"), proxyPath);
        ...
        try {
            ...
            expect(
                first.deployments.map(({ name, kind, action }) => ({
                    name,
                    kind,
                    action,
                })),
            ...
            expect(BigInt(await callFunction(rpc, proxy.slot, 1, "", "uint64"))).toBe(0n);
            ...
            expect(BigInt(await callFunction(rpc, proxy.slot, 1, "", "uint64"))).toBe(1n);
            ...
            expect(BigInt(await callFunction(rpc, proxy.slot, 1, "", "uint64"))).toBe(3n);
        } finally {
            handle.stop();
