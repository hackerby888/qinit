...
const queryIn = (v: bigint, ms: number) => {
...
const mockReply = (echoed: bigint, doubled: bigint) => {
  const b = new Uint8Array(16);
  const dv = new DataView(b.buffer);
...
test("oracle RPC seam: discover a pending query, inject the reply, the notification fires", async () => {
  ...
  try {
    ...
    const res = await post(h.rpcBase + "/live/v1/dev/oracle-resolve", {
      queryId: q.queryId,
      reply: Buffer.from(mockReply(21n, 42n)).toString("base64"),
    });
    ...
    const bad = await post(h.rpcBase + "/live/v1/dev/oracle-resolve", {
      queryId: "999",
      reply: Buffer.from(mockReply(1n, 2n)).toString("base64"),
    });
