...
export class QubicLogStore {
  ...
  begin(tick: number, txId: number): void {
    ...
    if (this.overflowedTick !== null) {
      throw new Error(
        `log retention limit exceeded in tick ${this.overflowedTick}`,
      );
    }
  ...
  private append(
    ...
    if (this.current.tick <= this.lastUpdatedTick) {
      throw new Error(
        `cannot write logs for finalized tick ${this.current.tick}`,
      );
    }
    ...
    if (contractIndex !== undefined && message.length >= 4) {
      new DataView(
        ...
        message.byteOffset,
        message.byteLength,
      ).setUint32(0, contractIndex >>> 0, true);
    }
    ...
    view.setUint32(
      6,
      (message.length & 0xffffff) | ((type & 0xff) << 24),
      true,
    );
  ...
  range(tick: number, txId: number): QubicLogRange {
    ...
    return (
      this.ranges.get(tick)?.[txId] ??
      { fromLogId: -2n, length: -2n }
    );
  }
