...
export class VirtualNode implements NodeTransport {
  ...
  // ---- Explorer read models. These mirror core-lite's /explorer/data and /query/v1/* response shapes so
  // the TUI explorer can run against either backend without branching. Fields the simulator genuinely has no
  // source for (peers, contract construction epochs) are reported as zeros rather than invented.
  ...
  private async explorerTx(record: TxRecord): Promise<ExplorerTx> {
    const raw = this.rawTx(record.txId);
    let inputSize = 0;
    let inputData = "";
    let signature = "";
  ...
  async explorerData(): Promise<ExplorerData> {
    const { tick, epoch, initialTick } = this.epochInfo();
  ...
  async explorerTickData(tick: number): Promise<ExplorerTickData | null> {
    const td = this.sim.tickData(tick);
    if (!td) {
      return null;
    }
    ...
    return {
      ...
      transactionDigests: this.sim
        .tickTransactions(tick)
  ...
  async explorerTickTransactions(tick: number): Promise<ExplorerTx[]> {
    ...
    return Promise.all(records.map((record) => this.explorerTx(record)));
  }

  async explorerTxByHash(hash: string): Promise<ExplorerTx | null> {
    const record = this.sim.txByHash(hash.toLowerCase());
    return record ? this.explorerTx(record) : null;
  }
  ...
  async explorerTransfersForIdentity(
    identity: string,
    direction: "in" | "out" | "both",
    limit: number,
  ): Promise<{ identity: string; count: number; transactions: IdentityTransfer[] }> {
    const target = toHex(this.idToBytes(identity));
  ...
  async explorerContractCalls(options: {
    fromTick: number;
    toTick: number;
    contractIndex?: number;
    page?: number;
    ...
    const transactions = await Promise.all(
      pageHits.map(async (hit) => ({
        ...(await this.explorerTx(hit.record)),
  ...
  async explorerContracts(): Promise<{ contracts: ContractListEntry[]; count: number }> {
    const registry = await this.dynRegistry();
