      ...
      fetch: async (req: Request): Promise<Response> => {
        ...
        try {
          if (path === "/tick-info" || path === "/latest-created-tick-info")
            return json(await eng.tickInfo());
          ...
          if (path === "/live/v1/dev/oracle-pending") {
            ...
            return json({
              queries: qs.map((qq) => ({
                queryId: qq.queryId.toString(),
                slot: qq.slot,
                interfaceIndex: qq.interfaceIndex,
                query: Buffer.from(qq.query).toString("base64"),
              })),
            });
          }
          ...
          if (path === "/query/v1/getTransactionsForTick" && req.method === "POST") {
            ...
            return json({
              transactions: await eng.tickTransactions(Number(body.tickNumber ?? body.tick ?? 0)),
            });
          }
          ...
          if (path === "/live/v1/broadcast-transaction" && req.method === "POST") {
            ...
            return json({
              ok: r.ok,
              peersBroadcasted: r.ok ? 1 : 0,
              transactionId: r.transactionId,
              code: r.ok ? undefined : 1,
              message: r.message,
            });
          }
          if (path === "/live/v1/querySmartContract" && req.method === "POST") {
            ...
            const out = await eng.querySmartContract(
          ...
          if (path === "/live/v1/dev/deploy" && req.method === "POST") {
            ...
            return json({
              ...
              digest: eng.sim.digest(Number(body.slot)),
            });
          }
