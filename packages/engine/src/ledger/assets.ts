...
export class AssetLedger {
    ...
    private setRecord(index: number, record: LedgerRecord): void {
        // The index probes walk until they find a hole, so a full table would spin forever: refuse the insert that would fill it and report the limit.
    ...
    private addOwnership(issuanceIndex: number, ownershipIndex: number): void {
        this.nextIndex.set(ownershipIndex, this.firstChildIndex.get(issuanceIndex) ?? NO_ASSET_INDEX);
        this.firstChildIndex.set(issuanceIndex, ownershipIndex);
    }

    private addPossession(ownershipIndex: number, possessionIndex: number): void {
        this.nextIndex.set(possessionIndex, this.firstChildIndex.get(ownershipIndex) ?? NO_ASSET_INDEX);
    ...
    private issuanceIndex(issuer: Id, name: bigint): number {
        ...
        for (;;) {
            ...
            if (record.type === ISSUANCE && record.name === name && first32BytesEqual(record.publicKey, issuer)) {
                return index;
            }
    ...
    issueAssetRaw(issuer: Id, name: bigint, decimals: number, unit: bigint, shares: bigint, managingContractIndex: number): bigint {
        ...
        this.host.logAssetMutation?.(QUBIC_LOG_TYPE.ASSET_ISSUANCE, encodeAssetIssuanceLog(issuer, shares, managingContractIndex, name, decimals, unit));
    ...
    issueAsset(slot: number, name: bigint, issuer: Id, decimals: number, shares: bigint, unit: bigint, invocator: Id): bigint {
        ...
        for (let index = 1; index < 7; index++) {
            const character = Number((name >> BigInt(index * 8)) & 0xffn);
            if (character === 0) {
                for (let tailIndex = index + 1; tailIndex < 7; tailIndex++) {
                    const tailCharacter = Number((name >> BigInt(tailIndex * 8)) & 0xffn);
                    if (tailCharacter !== 0) {
                        return 0n;
                    }
                }
        ...
        for (let index = 1; index < 7; index++) {
            ...
            const isUppercaseLetter = character >= 0x41 && character <= 0x5a;
        ...
        const isAuthorizedIssuer = first32BytesEqual(issuer, this.host.contractId(slot)) || first32BytesEqual(issuer, invocator);
    ...
    numberOfShares(assetBytes: Uint8Array, ownershipSelectionBytes: Uint8Array, possessionSelectionBytes: Uint8Array): bigint {
    ...
    private numberOfSharesSel(issuer: Id, name: bigint, ownership: AssetSelection, possession: AssetSelection): bigint {
        ...
        if (possession.anyId && possession.anyMgmt) {
            for (const ownershipIndex of this.ownershipIndices(issuanceIndex, ownership)) {
                total += this.record(ownershipIndex)!.shares;
            }
        } else {
            for (const ownershipIndex of this.ownershipIndices(issuanceIndex, ownership)) {
                for (const possessionIndex of this.possessionIndices(ownershipIndex, possession)) {
    ...
    enumerate(assetBytes: Uint8Array, ownershipSelectionBytes: Uint8Array, possessionSelectionBytes: Uint8Array, kind: number): AssetEntry[] {
        const asset = Asset.wrap(assetBytes);
        const ownership = parseSelect(ownershipSelectionBytes);
        const possession = parseSelect(possessionSelectionBytes);
    ...
    possessionsOf(issuer: Id, name: bigint): AssetEntry[] {
        ...
        for (const ownershipIndex of this.ownershipIndices(issuanceIndex, ANY_SELECT)) {
            const ownership = this.record(ownershipIndex)!;

            for (const possessionIndex of this.possessionIndices(ownershipIndex, ANY_SELECT)) {
    ...
    private transferShareOwnershipAndPossessionByIndex(sourceOwnershipIndex: number, sourcePossessionIndex: number, destination: Id, shares: bigint): boolean {
        ...
        if (isZeroId(destination)) {
            ...
            this.logOwnershipAndPossessionChange(sourceOwnership, sourcePossession, destination, issuance, shares);
        ...
        for (;;) {
            const destinationOwnership = this.record(destinationOwnershipIndex);
            if (
                !destinationOwnership ||
                (destinationOwnership.type === OWNERSHIP &&
                    destinationOwnership.mgmt === sourceOwnership.mgmt &&
                    destinationOwnership.crossRef === sourceOwnership.crossRef &&
                    first32BytesEqual(destinationOwnership.publicKey, destination))
            ) {
                break;
            }

            destinationOwnershipIndex = (destinationOwnershipIndex + 1) & ASSET_INDEX_MASK;
        }
        ...
        if (!destinationOwnership) {
            ...
            this.addOwnership(sourceOwnership.crossRef, destinationOwnershipIndex);
        } else {
        ...
        for (;;) {
            const destinationPossession = this.record(destinationPossessionIndex);
            if (
                !destinationPossession ||
                (destinationPossession.type === POSSESSION &&
                    destinationPossession.mgmt === sourcePossession.mgmt &&
                    destinationPossession.crossRef === destinationOwnershipIndex &&
                    first32BytesEqual(destinationPossession.publicKey, destination))
            ) {
                break;
            }

            destinationPossessionIndex = (destinationPossessionIndex + 1) & ASSET_INDEX_MASK;
        }
        ...
        if (!destinationPossession) {
            ...
            this.addPossession(destinationOwnershipIndex, destinationPossessionIndex);
        } else {
        ...
        this.logOwnershipAndPossessionChange(sourceOwnership, sourcePossession, destination, this.record(sourceOwnership.crossRef)!, shares);
    ...
    transferShareOwnershipAndPossession(slot: number, name: bigint, issuer: Id, owner: Id, possessor: Id, shares: bigint, newOwner: Id): bigint {
        ...
        for (;;) {
            ...
            if (record.type === OWNERSHIP && record.crossRef === issuanceIndex && first32BytesEqual(record.publicKey, owner) && record.mgmt === slot) {
                break;
            }
        ...
        for (;;) {
            ...
            if (possession.type === POSSESSION && possession.crossRef === ownershipIndex && first32BytesEqual(possession.publicKey, possessor)) {
    ...
    private transferShareManagementRightsByIndex(
        ...
        let destinationOwnershipIndex = this.startOf(sourceOwnership.publicKey);
        ...
        for (;;) {
            ...
            if (
                ...
                (destinationOwnership.type === OWNERSHIP &&
                    ...
                    destinationOwnership.crossRef === sourceOwnership.crossRef &&
                    first32BytesEqual(destinationOwnership.publicKey, sourceOwnership.publicKey))
            ) {
                break;
            }
        ...
        let destinationPossessionIndex = this.startOf(sourcePossession.publicKey);
        ...
        for (;;) {
            ...
            if (
                ...
                (destinationPossession.type === POSSESSION &&
                    ...
                    destinationPossession.crossRef === destinationOwnershipIndex &&
                    first32BytesEqual(destinationPossession.publicKey, sourcePossession.publicKey))
            ) {
                break;
            }
    ...
    assetUniverse(): AssetSnapshot[] {
        ...
        for (
            ...
            issuanceIndex = this.nextIndex.get(issuanceIndex) ?? NO_ASSET_INDEX
        ) {
    ...
    getUniverseDigest(): Uint8Array {
        if (!this.tree) {
            const emptyRecordDigest = k12Bytes(new Uint8Array(ASSET_RECORD_SIZE));
    ...
    universeProofIssuances(filter: AssetIssuanceFilter = {}): IssuedProof[] {
        this.getUniverseDigest();
        const proofs: IssuedProof[] = [];
        const requestedName = filter.name === undefined ? undefined : filter.name & 0xffffffffffffffn;
        ...
        for (
            ...
            const issuance = this.record(issuanceIndex)!;
            if (filter.issuer && !first32BytesEqual(issuance.publicKey, filter.issuer)) {
                continue;
            }
    ...
    universeProofOwnerships(filter: AssetOwnershipFilter): AssetRecordProof[] {
        this.getUniverseDigest();
        const issuanceIndex = this.issuanceIndex(filter.issuer, filter.name & 0xffffffffffffffn);
        if (issuanceIndex === NO_ASSET_INDEX) {
            return [];
        }
        ...
        return this.ownershipIndices(issuanceIndex, selection).map((index) => this.universeProof(index));
    }

    universeProofPossessions(filter: AssetPossessionFilter): AssetRecordProof[] {
        this.getUniverseDigest();
        const issuanceIndex = this.issuanceIndex(filter.issuer, filter.name & 0xffffffffffffffn);
        if (issuanceIndex === NO_ASSET_INDEX) {
            return [];
        ...
        for (const ownershipIndex of this.ownershipIndices(issuanceIndex, ownershipSelection)) {
            for (const possessionIndex of this.possessionIndices(ownershipIndex, possessionSelection)) {
