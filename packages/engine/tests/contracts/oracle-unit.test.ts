...
import {
  OracleQuery as MockOracleQuery,
  OracleReply as MockOracleReply,
} from "../../src/oracle-interfaces/mock";
import {
  OracleQuery as PriceOracleQuery,
  OracleReply as PriceOracleReply,
} from "../../src/oracle-interfaces/price";
...
test("one-time query charges once and delivers the typed reply", () => {
  ...
  expect(
    oracle.resolve(queryId, new Uint8Array(PriceOracleReply.SIZE).fill(9)),
  ).toBe(true);
  ...
  expect(notification.input.subarray(16)).toEqual(
    new Uint8Array(PriceOracleReply.SIZE).fill(9),
  );
});
...
test("query validates Core interface metadata before charging", () => {
  ...
  expect(
    oracle.query(7, 3, priceQuery(), PriceOracleReply.SIZE, 1, 1_000, 10n),
  ).toBe(-1n);
  expect(
    oracle.query(
      7,
  ...
  expect(
    oracle.query(
      7,
      1,
      new Uint8Array(MockOracleQuery.SIZE),
      MockOracleReply.SIZE,
      1,
      1_000,
      0n,
    ),
  ...
  expect(
    oracle.query(
      7,
      2,
      new Uint8Array(DogeShareValidationOracleQuery.SIZE),
      DogeShareValidationOracleReply.SIZE,
      1,
      1_000,
      99_999n,
    ),
...
test("subscription requires whole minutes and charges only the SUBSCRIBE call", () => {
  ...
  expect(
    new DataView(oracle.pending()[0].query.buffer).getBigUint64(
      PriceOracleQuery.OFFSETS.timestamp,
      true,
    ),
  ).toBe(packDateAndTime(host.clock));
...
test("subscribers share a channel, can receive its previous reply, and expire at epoch change", () => {
  ...
  expect(
    oracle.resolve(1n, new Uint8Array(PriceOracleReply.SIZE).fill(7)),
  ).toBe(true);
  const second = oracle.subscribe(
    6,
    0,
    query,
    PriceOracleReply.SIZE,
  ...
  expect(previous.input.subarray(16)).toEqual(
    new Uint8Array(PriceOracleReply.SIZE).fill(7),
  );
  expect(
    oracle.subscribe(
      6,
