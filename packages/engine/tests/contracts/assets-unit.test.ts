// AssetLedger (assets.ts) in isolation. A fake AssetHost supplies only contract-id derivation, so issuance, share transfer and merkle logic run directly.
...
test("universe proof helpers retain records, links, and managing contracts", () => {
    ...
    assets.transferShareOwnershipAndPossession(1, NAME, issuer, issuer, issuer, 300n, bob);
    ...
    expect(AssetRecord.wrap(legacyProof.issuanceRecord).unitOfMeasurement[0]).toBe(2);
    expect(assets.universeProofAt(ownerships[0].index)?.record).toEqual(ownerships[0].record);
