...
import {
  LOG_HEADER_SIZE,
  QubicLogStore,
} from "../../src/logging/qubic-log-store";
...
describe("Qubic log store", () => {
  ...
  test("encodes the core-lite record header and transaction range", () => {
    ...
    expect(view.getUint32(6, true)).toBe(
      (QUBIC_LOG_TYPE.CONTRACT_INFORMATION_MESSAGE << 24) |
        source.length,
    );
  ...
  test("rejects writes and duplicate finalization for a finalized tick", () => {
    ...
    expect(() => logger.begin(1, 0)).toThrow(
      "cannot write logs for finalized tick 1",
    );
    expect(() => logger.finalizeTick(1)).toThrow(
      "cannot finalize logs for tick 1 twice",
    );
  ...
  test("reset starts a new epoch with log ID zero and a fresh digest chain", () => {
    ...
    expect(logger.digest(20)).toEqual(
      k12Bytes(concatBytes([ZERO32, stampedMessage(28, 2)])),
    );
  });
  ...
  test("signals when one tick exceeds the retention limit", () => {
    ...
    expect(() => logger.finalizeTick(1)).toThrow(
      "log retention limit exceeded in tick 1",
    );
    expect(logger.recordsBetween(0n, 0n)).toBeNull();
