...
test("transactions log refunds and successful zero transfers", async () => {
  ...
  expect(logger.digest(1)).toEqual(
    k12Bytes(concatBytes([ZERO32, ...expectedMessages])),
  );
});
...
test("QPI transfer logs follow the destination callback logs", async () => {
  ...
  expect(logs[1].message).toEqual(
    quTransferMessage(source, destination, 5n),
  );
});
...
test("asset mutations emit exact native records only after success", () => {
  ...
  logger.begin(1, 0);
  expect(
    sim.host.issueAsset(28, name, issuer, 2, 1000n, unit, issuer),
  ).toBe(1000n);
  expect(
    sim.host.transferShares(
      28,
      name,
      issuer,
      issuer,
  ...
  expect(
    sim.transferShareManagementRights(
      name,
      issuer,
      holder,
      holder,
