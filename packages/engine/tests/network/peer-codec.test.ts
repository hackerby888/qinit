...
test("tick request decoders require the full Core flag layouts", () => {
  ...
  const transactionsRequest = new Uint8Array(
    codec.REQUEST_TICK_TRANSACTIONS_SIZE,
  );
  dv(transactionsRequest).setUint32(0, 456, true);
  transactionsRequest[4 + (4095 >> 3)] |= 1 << (4095 & 7);

  const transactions = codec.decodeTickTransactionsRequest(
    transactionsRequest,
  )!;
  expect(transactions.tick).toBe(456);
  expect(transactions.transactionFlags.length).toBe(
    codec.TICK_TRANSACTION_FLAGS_SIZE,
  );
  expect(
    transactions.transactionFlags[4095 >> 3] & (1 << (4095 & 7)),
  ).not.toBe(0);
  expect(
    codec.decodeTickTransactionsRequest(new Uint8Array(4)),
  ).toBeNull();
});
