...
function quorumTickRequest(tick: number, knownComputorIndices: number[] = []): Uint8Array {
...
function tickTransactionsRequest(tick: number, knownTransactionIndices: number[] = []): Uint8Array {
    const request = new Uint8Array(codec.REQUEST_TICK_TRANSACTIONS_SIZE);
    dv(request).setUint32(0, tick, true);
...
test("owned-assets request serves a merkle proof that recomputes the universe root", async () => {
    ...
    try {
        ...
        const issued = issuedFrames.find((frame) => frame.type === MSG.RESPOND_ISSUED_ASSETS)!;
        ...
        const modernFrames = await exchange(port, codec.frame(MSG.REQUEST_ASSETS, byIndex, 61));
        const modern = modernFrames.find((frame) => frame.type === MSG.RESPOND_ASSETS)!;
...
test("tick-transactions request honors transaction flags", async () => {
    ...
    try {
        ...
        expect(frames.some((frame) => frame.type === MSG.END_RESPONSE)).toBe(true);
    } finally {
        stop();
    }
});
