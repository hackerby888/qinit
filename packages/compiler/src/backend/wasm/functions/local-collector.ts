    ...
    switch (statement.kind) {
        ...
        case AstKind.DECLARATION: {
            ...
            if (statement.declaration.kind === AstKind.TYPEDEF_DECL) {
                ...
                if (!context.programAnalysis.typedefs.has(td.name)) context.programAnalysis.typedefs.set(td.name, td.type);
                break;
            }
                ...
                if (isAutoType(dType) && variableDeclaration.initializer) {
                    ...
                    if (ci) {
                        ...
                        const named = objectStruct?.members.find(
                            (member) => member.kind === AstKind.FUNCTION && (member as FunctionDecl).name === callee.member,
