...
export function collectFunctionLocals(
    statement: Statement,
        ...
        case AstKind.DECLARATION: {
            ...
            if (statement.declaration.kind === AstKind.STRUCT) {
                ...
                if (
                    ...
                    !context.programAnalysis.globalStructs.has(structDeclaration.name)
                )
                    context.programAnalysis.globalStructs.set(
                        structDeclaration.name,
                        structDeclaration,
                    );
                break;
            ...
            if (statement.declaration.kind === AstKind.VARIABLE) {
                ...
                const holdsAddr =
                    ...
                    /(ScopedScratchpad|Iterator)$/.test(variableDeclaration.type.name);
                ...
                if (isAutoType(dType) && variableDeclaration.initializer) {
                    ...
                    if (ci) {
                        ...
                        dType =
                            context.localVars.get(variableDeclaration.initializer.name)?.type ??
                            context.params?.get(variableDeclaration.initializer.name)?.type ??
                            dType;
                    } else if (
                        ...
                        dType =
                            context.programAnalysis.helpers.get(
                                variableDeclaration.initializer.callee.name,
                            )?.retType ?? dType;
                    } else if (
                        variableDeclaration.initializer.kind === AstKind.CALL &&
                        ...
                        const objectType =
                            context.localVars.get(objectName)?.type ??
                            context.params?.get(objectName)?.type;
                        const objectStruct = objectType
                            ? context.programAnalysis.structOf(objectType, templateBindings)
                            : null;
                        const named = objectStruct?.members.find(
                            (member) =>
                                member.kind === AstKind.FUNCTION &&
                                (member as FunctionDecl).name === callee.member,
                        ) as FunctionDecl | undefined;
                        dType = named?.returnType ?? dType;
                    } else if (
                        variableDeclaration.initializer.kind === AstKind.SUBSCRIPT &&
                        variableDeclaration.initializer.object.kind === AstKind.IDENTIFIER
                    ) {
                        const ot = context.localVars.get(
                            variableDeclaration.initializer.object.name,
                        )?.type;
                ...
                const concrete =
                    dType.kind === AstKind.NAME && templateBindings.types.has(dType.name)
                        ? templateBindings.types.get(dType.name)!
                        : dType;
                const isAgg =
                    !holdsAddr &&
                ...
                if (!context.localVars.has(variableDeclaration.name)) {
                    context.localVars.set(variableDeclaration.name, {
                        wasmType,
                        type: resolveAliasType(context.programAnalysis, concrete),
                    });
                }
            }
            break;
