...
export function emitDiscardedExpression(
...
export function emitIncrementOrDecrement(
    context: FunctionEmissionContext,
    expression: Expression,
): string {
    const argument =
        expression.kind === AstKind.POSTFIX_OP || expression.kind === AstKind.PREFIX_OP
            ? expression.argument
            : expression;
    const operator = (expression as any).operator === UpdateOp.INCREMENT ? "i64.add" : "i64.sub";
    ...
    if (argument.kind === AstKind.IDENTIFIER && isScalarLocal(context, argument.name)) {
        const next = watIr.operation(
            operator,
            watIr.localGet(argument.name, WatNodeType.I64),
            watIr.i64Constant(1),
        );
        ...
        if (isUint128(context.programAnalysis, addr.type ?? null)) {
            if ((expression as any).operator === UpdateOp.INCREMENT) {
                ...
                return watIr.serializeWatNode(
                    watIr.operation(
                        "drop",
                        watIr.functionCallWithSignature(
                            { params: [WatNodeType.I32], res: WatNodeType.I32 },
                            compiled.cm.label,
                            addrIr(addr.addr),
                        ),
                    ),
            ...
            const res = context.lowering.lowerUint128Expression(context, {
                ...
                operator: operator === "i64.add" ? BinaryOp.ADD : BinaryOp.SUBTRACT,
                left: argument,
                right: one,
                span: (expression as any).span,
            });
            return watIr.serializeWatNode(
                watIr.functionCall("$copyMem", addrIr(addr.addr), res, watIr.i32Constant(16)),
            );
        }
        const load = emitScalarLoad(
            addr.addr,
            addr.size,
            isSignedScalarType(addr.type, context.programAnalysis),
        );
