...
export function isU128Expr(context: FunctionEmissionContext, expression: Expression): boolean {
    ...
    if (expression.kind === AstKind.CONSTRUCT) return isUint128(context.programAnalysis, expression.type);
    if (expression.kind === AstKind.C_CAST || expression.kind === AstKind.STATIC_CAST) {
        const type = (expression as any).type;
        if (type?.kind === AstKind.NAME && (type.name === "uint128" || type.name === "uint128_t")) return true;
        return isU128Expr(context, expression.expression);
    }
    if (expression.kind === AstKind.TERNARY) return isU128Expr(context, expression.then) || isU128Expr(context, expression.else_);
    if (expression.kind === AstKind.TEMPLATE_CALL && expression.callee.kind === AstKind.IDENTIFIER) {
        ...
        if ((base === "div" || base === "mod") && MATH_INTRINSIC_NAMES.has(base) && expression.callArguments.length === 2) {
            const ta = expression.templateArguments?.[0];
            if (ta?.kind === AstKind.NAME && (ta.name === "uint128" || ta.name === "uint128_t")) return true;
    ...
    if (expression.kind === AstKind.CALL && expression.callee.kind === AstKind.IDENTIFIER) {
        ...
        if (bound && isUint128(context.programAnalysis, bound)) return true;
    ...
    if (expression.kind === AstKind.BINARY_OP) {
        if (expression.operator === BinaryOp.SHIFT_LEFT || expression.operator === BinaryOp.SHIFT_RIGHT) return isU128Expr(context, expression.left);
        if (
            expression.operator === BinaryOp.MULTIPLY ||
    ...
    if (expression.kind === AstKind.CALL && expression.callee.kind === AstKind.MEMBER_ACCESS) {
        ...
        const fn = struct?.members.find(
            (member) =>
                ...
                (member as FunctionDecl).name ===
                    (
                        expression.callee as Expression & {
...
export function sourceU128Result(
...
export function lowerUint128Expression(context: FunctionEmissionContext, expression: Expression): watIr.WatNode {
    ...
    if (expression.kind === AstKind.CONSTRUCT && isUint128(context.programAnalysis, expression.type)) return constructU128(context, expression.callArguments);
    if (expression.kind === AstKind.C_CAST || expression.kind === AstKind.STATIC_CAST) {
    ...
    if (expression.kind === AstKind.TERNARY) {
        const destination = context.lowering.allocateScratchSlotNode(context, 16);
        context.lines.push(
            `    (if ${watIr.serializeWatNode(watIr.operation("i64.ne", watIr.i64Constant(0), context.lowering.lowerValueExpression(context, expression.condition)))} (then`,
        );
        context.lines.push(
    ...
    if (expression.kind === AstKind.BINARY_OP) {
        ...
        if (method) {
            ...
            const key =
                scalarRight && expression.operator === BinaryOp.BITWISE_AND
                    ? "int"
