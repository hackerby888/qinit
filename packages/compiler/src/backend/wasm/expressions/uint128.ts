...
export function isU128Expr(context: FunctionEmissionContext, expression: Expression): boolean {
    ...
    if (
        ...
        if (
            ...
            if (ta?.kind === AstKind.NAME && (ta.name === "uint128" || ta.name === "uint128_t"))
                return true;
            return (
                isU128Expr(context, expression.callArguments[0]) ||
                isU128Expr(context, expression.callArguments[1])
            );
        }
    }
    ...
    if (expression.kind === AstKind.CALL && expression.callee.kind === AstKind.MEMBER_ACCESS) {
        ...
        for (let index = 0; index < 8 && ot?.kind === AstKind.NAME; index++) {
            const next =
                context.thisBind?.types.get(ot.name) ??
                context.programAnalysis.typedefs.get(ot.name);
            if (!next) break;
            ot = next;
        }
        ...
        const struct = ot
            ? context.programAnalysis.structOf(ot, context.thisBind ?? EMPTY_TEMPLATE_BINDINGS)
            : null;
...
export function constructU128(
    ...
    const compiled = context.lowering.callCompiled(
        ...
        U128_CLASS,
        "uint128_t",
        watIr.serializeWatNode(destination),
        callArguments,
    );
...
export function sourceU128Result(
    ...
    if (compiled.retDest) {
        ...
        return watIr.rawWatNode(
            compiled.retDest,
            WatNodeType.I32,
            "source-compiled uint128 aggregate result",
        );
    }
    if (compiled.cm.retKind === WatNodeType.I64)
        return watIr.rawWatNode(
            compiled.call,
            WatNodeType.I64,
            "source-compiled uint128 scalar result",
        );
    if (compiled.cm.retKind === WatNodeType.I32)
        return watIr.rawWatNode(
            compiled.call,
            WatNodeType.I32,
            "source-compiled uint128 reference result",
        );
    context.lines.push(`    ${compiled.call}`);
...
export function lowerUint128Expression(
    context: FunctionEmissionContext,
    ...
    if (
        ...
        isUint128(context.programAnalysis, expression.type)
    )
    ...
    if (expression.kind === AstKind.CALL && expression.callee.kind === AstKind.IDENTIFIER) {
        ...
        if (
            ...
            return watIr.rawWatNode(
                context.lowering.emitAggHelperCall(context, expression, helper),
                WatNodeType.I32,
    ...
    if (
        ...
        return watIr.rawWatNode(
            context.lowering.emitAggHelperCall(context, callExpr, helper),
            WatNodeType.I32,
            "source-compiled uint128 div result",
        );
    }
