...
export function lowerValueExpression(
    ...
    if (
        ...
        return watIr.operation(
            "i64.extend_i32_u",
            watIr.functionCall(
                "$qt_construction_epoch",
                watIr.operation(
                    "i32.wrap_i64",
                    lowerValueExpression(context, expression.object.index),
                ),
            ),
        );
    }
    if (
        ...
        return watIr.operation(
            "i64.extend_i32_u",
            watIr.localGet(expression.object.name, WatNodeType.I32),
        );
    }
    ...
    if (
        ...
        const result = context.lowering.sourceU128Result(
            context,
            "operator bool",
            context.lowering.lowerUint128Expression(context, expression),
            [],
        );
    ...
    switch (expression.kind) {
        ...
        case AstKind.IDENTIFIER: {
            ...
            if (
                context.localVars.has(expression.name) &&
                !context.refLocals?.has(expression.name)
            ) {
                return watIr.localGet(
                    expression.name,
                    context.localVars.get(expression.name)!.wasmType,
                );
            }
            ...
            if (context.refLocals?.get(expression.name)?.kind === AstKind.POINTER) {
                return watIr.operation(
                    "i64.extend_i32_u",
                    watIr.localGet(expression.name, WatNodeType.I32),
                );
            }
            ...
            if (expression.name.startsWith("__id_")) {
                ...
                if (line !== undefined)
                    return watIr.i64Constant(
                        (context.programAnalysis.slot << 22) | (line & 0x3fffff),
                    );
            }
            ...
            if (context.thisLayout) {
                ...
                if (tn && tn.size <= 8)
                    return lowerScalarLoad(
                        tn.addr,
                        tn.size,
                        isSignedScalarType(tn.type, context.programAnalysis),
                    );
            }
            // Load scalar-typedef entry I/O through its region address.
            if (
                (expression.name === "input" || expression.name === "output") &&
                !context.localVars.has(expression.name)
            ) {
                ...
                if (
                    ...
                    io.size > 0 &&
                    io.size <= 8 &&
                    (!io.layout || io.layout.fields.size === 0)
                ) {
                    return lowerScalarLoad(
                        io.addr,
                        io.size,
                        isSignedScalarType(io.type, context.programAnalysis),
                    );
                }
            }
        ...
        case AstKind.MEMBER_ACCESS: {
            ...
            if (resolvedAddress && resolvedAddress.size <= 8)
                return lowerScalarLoad(
                    ...
                    resolvedAddress.size,
                    isSignedScalarType(resolvedAddress.type, context.programAnalysis),
                );
            if (resolvedAddress) {
                context.programAnalysis.warn(
                    `aggregate value read unsupported [${describeShape(expression)}]`,
            ...
            if (ot?.kind === AstKind.TEMPLATE_INSTANCE) {
                const sc = context.programAnalysis.staticConstsOf(
                    ot.name,
                    context.programAnalysis.bindContainer(ot.name, ot.callArguments),
                );
            ...
            context.programAnalysis.warn(
                `unsupported member read [${describeShape(expression)}]`,
                expression.span.line,
            );
            return watIr.i64Constant(0);
        }
        case AstKind.SUBSCRIPT: {
            ...
            context.programAnalysis.warn(
                `unsupported subscript value`,
                (expression as any).span?.line ?? 0,
            );
            return watIr.i64Constant(0);
        }
        case AstKind.CALL: {
            ...
            if (expression.callee.kind === AstKind.IDENTIFIER) {
                ...
                if (helper !== null)
                    return watIr.rawWatNode(
                        helper,
                        WatNodeType.I64,
                        "source-compiled template helper",
                    );
            }
            if (
                ...
                const source = context.lowering.emitTemplateContainerCall(
                    context,
                    expression,
                    true,
                );
                if (source !== null)
                    return watIr.rawWatNode(
                        source,
                        WatNodeType.I64,
                        "source-compiled template instance method",
                    );
            }
            context.programAnalysis.warn(
        ...
        case AstKind.UNARY_OP: {
            ...
            if (expression.operator === UnaryOp.DEREFERENCE) {
                const resolvedAddress = context.lowering.resolveExpressionAddress(
                    context,
                    expression,
                );
                if (resolvedAddress && resolvedAddress.size <= 8) {
            ...
            switch (expression.operator) {
                ...
                case UnaryOp.LOGICAL_NOT:
                    return watIr.operation(
                        "i64.extend_i32_u",
                        watIr.operation("i64.eqz", valueNode),
                    );
                default:
                    return valueNode;
            }
        ...
        case AstKind.TERNARY: {
            ...
            if (
                ...
                return narrowCastIr(
                    watIr.selectValue(
                        thenV,
                        elseV,
                        watIr.operation("i64.ne", watIr.i64Constant(0), condition),
                    ),
                    cvName,
                );
            }
            ...
            const thenB = [
                ...thenLines,
                `      ${context.lowering.setLocal(context, branchResultLocal, thenV)}`,
            ].join("\n");
            const elseB = [
                ...elseLines,
                `      ${context.lowering.setLocal(context, branchResultLocal, elseV)}`,
            ].join("\n");
            context.lines.push(
        ...
        case AstKind.CONSTRUCT: {
            ...
            if (
                ...
                SCALAR_SIZE[storageType.name] !== undefined &&
                SCALAR_SIZE[storageType.name] <= 8
            ) {
                return narrowCastIr(
                    expression.callArguments[0]
                        ? lowerValueExpression(context, expression.callArguments[0])
                        : watIr.i64Constant(0),
                    storageType.name,
                );
            }
            context.programAnalysis.warn(
                `aggregate construction used as a scalar value`,
                expression.span,
            );
            return watIr.i64Constant(0);
        }
        case AstKind.INITIALIZER_LIST:
            return expression.expressions.length === 1
                ? lowerValueExpression(context, expression.expressions[0])
                : watIr.i64Constant(0);
        ...
        default:
            context.programAnalysis.warn(
                `unsupported expression '${expression.kind}' as value`,
                (expression as any).span?.line ?? 0,
            );
            return watIr.i64Constant(0);
    }
...
export function aggOperand(
    ...
    const resolvedAddress = context.lowering.resolveExpressionAddress(context, expression);
    if (resolvedAddress)
        return resolvedAddress.size > 8
            ? { addr: resolvedAddress.addr, size: resolvedAddress.size }
            : null;
