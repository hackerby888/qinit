...
export function lowerValueExpression(context: FunctionEmissionContext, expression: Expression): watIr.WatNode {
    ...
    switch (expression.kind) {
        ...
        case AstKind.IDENTIFIER: {
            ...
            if (expression.name === "SELF_INDEX") return watIr.operation("i64.extend_i32_u", watIr.functionCall("$qpi_contractIndex"));
            ...
            if (expression.name.startsWith("__id_")) {
                ...
                if (line !== undefined) return watIr.i64Constant((context.programAnalysis.slot << 22) | (line & 0x3fffff));
            }
            ...
            if (context.thisLayout) {
                ...
                if (tn && tn.size <= 8) return lowerScalarLoad(tn.addr, tn.size, isSignedScalarType(tn.type, context.programAnalysis));
            }
        ...
        case AstKind.MEMBER_ACCESS: {
            ...
            for (let index = 0; index < 8 && ot?.kind === AstKind.NAME; index++) ot = context.programAnalysis.typedefs.get(ot.name) ?? null;
        ...
        case AstKind.UNARY_OP: {
            ...
            const canon32 = (count: watIr.WatNode) =>
                mask32 ? watIr.operation("i64.and", count, watIr.i64Constant("0xffffffff")) : sext32 ? watIr.operation("i64.extend32_s", count) : count;
        ...
        case AstKind.TERNARY: {
            ...
            context.lines = saved;
            if (thenLines.length === 0 && elseLines.length === 0 && watIr.isPureWatNode(thenV) && watIr.isPureWatNode(elseV)) {
