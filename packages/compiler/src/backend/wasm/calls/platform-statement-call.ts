...
export function tryEmitPlatformStatementCall(context: FunctionEmissionContext, expression: CallExpression): boolean {
    ...
    if (primitive?.kind === PlatformPrimitiveKind.MEMORY_STORE) {
        const destinationAddress = context.lowering.emitAddress(context, expression.callArguments[0]);
        ...
        const copyCall = watIr.functionCall("$copyMem", addrIr(destinationAddress), addrIr(sourceAddress), watIr.i32Constant(32));
