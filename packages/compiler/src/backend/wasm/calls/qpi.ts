...
export function materializeSelect(
    ...
    if (!expression || staticName === "any") {
        ...
        const source = callExpression?.callArguments[0]
            ? context.lowering.emitAddress(context, callExpression.callArguments[0])
            : null;
        if (!source) throw new Error(`${staticName} selector id is not addressable`);
        context.lines.push(
            `    ${watIr.serializeWatNode(watIr.functionCall("$copyMem", slot, addrIr(source), watIr.i32Constant(32)))}`,
        );
        flag(parsed.field("anyManagingContract"), 1);
        ...
        clamp(
            callExpression.callArguments[0],
            "i32.store16",
            parsed.field("managingContract"),
            "0xffff",
        );
        flag(parsed.firstField("anyOwner", "anyPossessor", "anyId"), 1);
    } else if (expression.kind === AstKind.INITIALIZER_LIST) {
        const source = expression.expressions[0]
            ? context.lowering.emitAddress(context, expression.expressions[0])
            : null;
        if (source)
            context.lines.push(
                `    ${watIr.serializeWatNode(watIr.functionCall("$copyMem", slot, addrIr(source), watIr.i32Constant(32)))}`,
        ...
        if (expression.expressions[3])
            clamp(
                ...
                "i32.store8",
                parsed.field("anyManagingContract"),
                "1",
            );
    } else {
...
export function materializeAssetAddress(
    ...
    if (expression?.kind === AstKind.INITIALIZER_LIST) {
        ...
        const issuer = expression.expressions[0]
            ? context.lowering.emitAddress(context, expression.expressions[0])
            : null;
        if (!issuer) throw new Error(`${bindingName} asset issuer is not addressable`);
        context.lines.push(
            `    ${watIr.serializeWatNode(watIr.functionCall("$copyMem", slot, addrIr(issuer), watIr.i32Constant(32)))}`,
        );
    ...
    const address = expression
        ? (context.lowering.resolveExpressionAddress(context, expression)?.addr ??
          context.lowering.emitAddress(context, expression))
        : null;
...
export function emitQpiCall(
    ...
    if (
        ...
        context.programAnalysis.hasInstanceMethod(
