...
export function materializeSelect(context: FunctionEmissionContext, expression: Expression | undefined): string {
    ...
    const clamp = (value: Expression, mnemonic: string, offset: number, mask: string) =>
        context.lines.push(
            `    ${watIr.serializeWatNode(watIr.rawStore(mnemonic, null, watIr.addressWithOffset(slot, offset), watIr.operation("i32.and", watIr.operation("i32.wrap_i64", context.lowering.lowerValueExpression(context, value)), watIr.i32Constant(mask))))}`,
        );
    const callExpression = expression?.kind === AstKind.CALL ? expression : undefined;
    const staticName = callExpression
        ? callExpression.callee.kind === AstKind.QUALIFIED_NAME
            ? callExpression.callee.name
            : callExpression.callee.kind === AstKind.MEMBER_ACCESS
    ...
    if (!expression || staticName === "any") {
        ...
        if (!callExpression?.callArguments[0]) throw new Error("byManagingContract selector is missing its contract index");
        clamp(callExpression.callArguments[0], "i32.store16", parsed.field("managingContract"), "0xffff");
        flag(parsed.firstField("anyOwner", "anyPossessor", "anyId"), 1);
    } else if (expression.kind === AstKind.INITIALIZER_LIST) {
        const source = expression.expressions[0] ? context.lowering.emitAddress(context, expression.expressions[0]) : null;
        if (source) context.lines.push(`    ${watIr.serializeWatNode(watIr.functionCall("$copyMem", slot, addrIr(source), watIr.i32Constant(32)))}`);
        else if (expression.expressions[0]) throw new Error("asset selector id is not addressable");
        if (expression.expressions[1]) clamp(expression.expressions[1], "i32.store16", parsed.field("managingContract"), "0xffff");
        if (expression.expressions[2]) clamp(expression.expressions[2], "i32.store8", parsed.firstField("anyOwner", "anyPossessor", "anyId"), "1");
        if (expression.expressions[3]) clamp(expression.expressions[3], "i32.store8", parsed.field("anyManagingContract"), "1");
    } else {
...
export function materializeAssetAddress(context: FunctionEmissionContext, expression: Expression | undefined, bindingName: string): string {
    if (expression?.kind === AstKind.INITIALIZER_LIST) {
        ...
        context.lines.push(
            `    ${watIr.serializeWatNode(watIr.rawStore("i64.store", null, watIr.addressWithOffset(slot, parsed.field("assetName")), context.lowering.lowerValueExpression(context, expression.expressions[1])))}`,
        );
        return watIr.serializeWatNode(slot);
    }
...
export function emitQpiCall(
    context: FunctionEmissionContext,
    expression: Expression & {
        kind: AstKind.CALL;
    },
): null {
    if (!(expression.callee.kind === AstKind.MEMBER_ACCESS && expression.callee.object.kind === AstKind.IDENTIFIER && expression.callee.object.name === "qpi"))
        return null;
    const contextType = context.params?.get("qpi")?.type;
    if (contextType?.kind === AstKind.NAME && context.programAnalysis.hasInstanceMethod(contextType.name, expression.callee.member)) return null;
    if (
