...
export function compileLibraryFunction(
    ...
    try {
        ...
        const wat = emitHelperFunction(programAnalysis, info, fn, {
            size: 0,
            align: 1,
            fields: new Map(),
        });
    ...
    const score = (def: FunctionTemplateDecl): number => {
        ...
        for (let index = 0; index < fps.length && index < callArguments.length; index++) {
            ...
            if (pat.kind === AstKind.NAME) {
                ...
                else if (at.kind === AstKind.NAME && at.name === pat.name) size += 2;
                continue;
            }
...
export function compileLibraryFunctionInstance(
    ...
    const keyArgs = def.params
        .map((parameter) =>
            parameter.kind === AstKind.TYPE
                ? programAnalysis.typeKeyOf(
                      bind.types.get(parameter.name) ?? {
                          kind: AstKind.NAME,
                          name: parameter.name,
                      },
                  )
