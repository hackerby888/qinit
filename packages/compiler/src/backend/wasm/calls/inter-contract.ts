...
export function emitInterContract(
    ...
    if (idx === null) {
        const resolvedConstant = context.programAnalysis.resolveConst(
            `${calleeArg.name}_CONTRACT_INDEX`,
        );
        if (resolvedConstant !== null) idx = Number(resolvedConstant);
    }
    const entry = isInvoke
        ? callee?.procedures[functionArg.name]
        : callee?.functions[functionArg.name];
    ...
    if (!inAddr || !outAddr)
        throw new Error(
            `${isInvoke ? "INVOKE" : "CALL"}_OTHER input and output must be addressable`,
        );
    const inSize =
        (expression.callArguments[2]
            ? context.lowering.resolveExpressionAddress(context, expression.callArguments[2])?.size
            : undefined) ?? entry.inSize;
    const outSize =
        (expression.callArguments[3]
            ? context.lowering.resolveExpressionAddress(context, expression.callArguments[3])?.size
            : undefined) ?? entry.outSize;
    ...
    if (isInvoke) {
        const reward = expression.callArguments[4]
            ? context.lowering.emitValue(context, expression.callArguments[4])
            : "(i64.const 0)";
