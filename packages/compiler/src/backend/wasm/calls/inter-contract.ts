...
export function emitInterContract(
    ...
    if (calleeArg?.kind !== AstKind.IDENTIFIER || functionArg?.kind !== AstKind.IDENTIFIER) return null;
    ...
    const inSize =
        (expression.callArguments[2] ? context.lowering.resolveExpressionAddress(context, expression.callArguments[2])?.size : undefined) ?? entry.inSize;
    const outSize =
        (expression.callArguments[3] ? context.lowering.resolveExpressionAddress(context, expression.callArguments[3])?.size : undefined) ?? entry.outSize;
