...
export function emitProposalProxyCall(
    ...
    const xExpr = (
        (expression.callee as any).object as Expression & {
            kind: AstKind.CALL;
        }
    ).callArguments[0];
...
export function emitProxySiblingCall(
    ...
    if (!context.proxyClass || expression.callee.kind !== AstKind.IDENTIFIER) return null;
    const method = expression.callee.name;
    const known =
        context.programAnalysis.templateMethods.get(context.proxyClass)?.has(method) ||
    ...
    if (!pvType || pvType.kind !== AstKind.TEMPLATE_INSTANCE) return null;
...
export function callProxy(
    ...
    if (valueWanted) {
        if (cm.retKind !== WatNodeType.I64) throw new Error("void proposal method used as a value");
        return call;
    }
    context.lines.push(
