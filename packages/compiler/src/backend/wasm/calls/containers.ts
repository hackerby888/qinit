...
export function compileContainerMethod(
    ...
    const explicitTemplateKey = explicitTemplateArgs
        .map((argument) => programAnalysis.typeKeyOf(argument))
        .join(",");
    ...
    const methodTemplateTypeKey = resolvedMethod.requiresMethodTemplateInference
        ? resolvedMethodArgumentTypes
              .map((argumentType) => (argumentType ? programAnalysis.typeKeyOf(argumentType) : "?"))
              .join(",")
        : "";
    ...
    if (
        resolvedMethod.requiresMethodTemplateInference &&
        definition.params.some((param) => param.kind === AstKind.TYPE)
    ) {
        const types = new Map(ownerBindings.types);
        const templateTypeNames = new Set(
            definition.params
                .filter((param) => param.kind === AstKind.TYPE)
    ...
    const functionParameters = (definition.functionParameters ?? []).map((parameter) =>
        classifyMethodParam(programAnalysis, parameter, ownerBindings),
    );
    const retType = programAnalysis.substInBindings(
        programAnalysis.derefType(definition.returnType),
        ownerBindings,
    );
    const returnsAddr =
        definition.returnType.kind === AstKind.REFERENCE ||
        definition.returnType.kind === AstKind.POINTER;
    const returnsAggregate =
        !returnsAddr &&
        !programAnalysis.isVoidType(definition.returnType) &&
        programAnalysis.isAggregateType(retType);
    const retKind: WatNodeType = returnsAddr
        ? WatNodeType.I32
        : programAnalysis.isVoidType(definition.returnType) || returnsAggregate
          ? WatNodeType.VOID
          : WatNodeType.I64;
    const retAgg = returnsAggregate
        ? programAnalysis.sizeOfType(retType, ownerBindings)
        : undefined;
    ...
    try {
        ...
        programAnalysis.warn(
            `failed to compile ${cacheKey}: ${entry.message}`,
            definition.span?.line ?? 0,
        );
...
function methodTypeKey(
    ...
    const argumentKeys = type.callArguments
        .map((argument) => context.typeKeyOf(argument))
        .join(",");
...
export function emitTemplateMethod(
    ...
    for (const fnParam of cm.functionParameters)
        context.params!.set(fnParam.name, {
            ...
            type:
                fnParam.concreteType ??
                programAnalysis.substInBindings(programAnalysis.derefType(fnParam.type), bind),
        });
    ...
    const result =
        cm.retKind === WatNodeType.I64
            ...
            : cm.retKind === WatNodeType.I32
              ...
              : "";
    const header =
        `  (func ${cm.label} ${retParam}(param $this i32) ${paramDecls}${result}`.replace(
            /\s+\)/,
            ")",
        );
...
export function callCompiled(
    ...
    if (!cm) return null;
    const minimumArgs = cm.functionParameters.findIndex(
    ...
    if (callArguments.length < minimum || callArguments.length > cm.functionParameters.length) {
        const expected =
            minimum === cm.functionParameters.length
                ? `${minimum}`
                : `${minimum}..${cm.functionParameters.length}`;
        throw new Error(
            `${type.name}::${method} expects ${expected} argument(s), got ${callArguments.length}`,
        );
    }
    ...
    const methodArgumentOperands = cm.functionParameters.map(
        (methodParameter, methodParameterIndex) => {
            const callArgument =
                callArguments[methodParameterIndex] ?? methodParameter.defaultValue;
            if (!callArgument) {
                throw new Error(
                    `${type.name}::${method} is missing required argument ${methodParameterIndex + 1}`,
                );
            }
            ...
            const paramType =
                methodParameter.concreteType ??
                context.programAnalysis.substInBindings(
            ...
            if (context.programAnalysis.isAggregateType(paramType)) {
                ...
                if (!direct)
                    throw new Error(
                        `${type.name}::${method} aggregate argument ${methodParameterIndex + 1} is not addressable`,
                    );
                return direct;
            }
            return context.lowering.argAddr(
    ...
    let retDest = "";
    if (cm.retAgg)
        retDest = watIr.serializeWatNode(
            context.lowering.allocateScratchSlotNode(context, cm.retAgg),
        );
    return {
...
export function emitTemplateContainerCall(
    ...
    if (
        ...
        (context.programAnalysis.globalStructs.has(type.name) ||
            context.programAnalysis.templateMethods.has(type.name))
    ) {
    ...
    context.lines.push(
        compiled.cm.retKind === WatNodeType.VOID
            ? `    ${compiled.call}`
            : `    (drop ${compiled.call})`,
    );
    return "";
...
export function emitContainerCall(
    ...
    if (valueWanted) {
        ...
        if (compiled.cm.retKind === WatNodeType.I32) {
            if (
                !compiled.cm.retType ||
                context.programAnalysis.isAggregateType(compiled.cm.retType)
            ) {
                throw new Error(
                    `aggregate reference ${node.type.name}::${member} used as a scalar`,
                );
            }
            return emitScalarLoad(
                compiled.call,
...
export function emitAssetIter(
    ...
    if (method === "begin") {
        const selN = watIr.rawWatNode(
            context.lowering.materializeSelect(context, undefined),
            WatNodeType.I32,
        );
        const asset = context.lowering.materializeAssetAddress(
            context,
        ...
        context.lines.push(
            `    ${watIr.serializeWatNode(watIr.rawStore("i32.store", null, itN, enumerate))}`,
        );
        context.lines.push(
            `    ${watIr.serializeWatNode(watIr.rawStore("i32.store", null, watIr.addressWithOffset(itN, 4), watIr.i32Constant(0)))}`,
        );
        return "";
    }
    if (method === "next") {
