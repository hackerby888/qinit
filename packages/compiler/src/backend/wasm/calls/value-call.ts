...
export function emitCallValueIr(
    ...
    if (
        ...
        if (
            (type.kind === AstKind.IDENTIFIER || type.kind === AstKind.QUALIFIED_NAME) &&
            member.kind === AstKind.IDENTIFIER
        ) {
            const field = context.programAnalysis.fieldOf(
                { kind: AstKind.NAME, name: type.name },
                member.name,
                context.thisBind,
            );
            if (field) return watIr.i64Constant(field.offset);
        }
    ...
    if (
        ...
        expression.callee.kind === AstKind.IDENTIFIER &&
        expression.callee.name === "getBalance"
    ) {
        const who = expression.callArguments[0]
    ...
    if (primitive?.kind === PlatformPrimitiveKind.MULTIPLY_HIGH) {
        const left = expression.callArguments[0]
            ? context.lowering.lowerValueExpression(context, expression.callArguments[0])
            : watIr.i64Constant(0);
        const right = expression.callArguments[1]
            ? context.lowering.lowerValueExpression(context, expression.callArguments[1])
            : watIr.i64Constant(0);
        const high = watIr.functionCall(
            primitive.signed ? "$intr_mulhi_s" : "$intr_mulhi_u",
            left,
            right,
        );
        let output: Expression | undefined = expression.callArguments[2];
        while (
            output?.kind === AstKind.PAREN ||
            (output?.kind === AstKind.UNARY_OP && output.operator === UnaryOp.ADDRESS_OF)
        ) {
        ...
        if (
            ...
            const out = expression.callArguments[2]
            ...
            context.lines.push(
                `    ${watIr.serializeWatNode(watIr.rawStore("i64.store", null, addrIr(out), high))}`,
            );
        }
    ...
    if (primitive?.kind === PlatformPrimitiveKind.CHAIN_RDRAND && primitive.width) {
        ...
        return watIr.operation(
            "i64.extend_i32_u",
            watIr.functionCall(`$intr_rdrand${primitive.width}`, addrIr(output)),
        );
    }
    if (primitive?.kind === PlatformPrimitiveKind.MASK_EXTRACT) {
        ...
        for (let byte = 0; byte < 32; byte++) {
            ...
            const bit = watIr.operation(
                "i64.and",
                watIr.operation("i64.shr_u", value, watIr.i64Constant(7)),
                watIr.i64Constant(1),
            );
            mask = watIr.operation(
                "i64.or",
                mask,
                watIr.operation("i64.shl", bit, watIr.i64Constant(byte)),
            );
        }
        return mask;
    }
    if (primitive?.kind === PlatformPrimitiveKind.TEST_ZERO) {
        ...
        for (let lane = 0; lane < 4; lane++) {
            ...
            combined = watIr.operation(
                "i64.or",
                combined,
                watIr.operation("i64.and", argument, templateBindings),
            );
        }
    ...
    }
    {
        ...
        if (wrapperMethod) {
            ...
            throw new Error(
                `authoritative proposal method '${wrapperMethod}' could not be lowered`,
            );
        }
    }
    ...
    if (
        ...
        const wat = context.lowering.emitInterContract(
            context,
            expression,
            expression.callee.name === "__qpi_invoke_other",
        );
        if (wat)
            return watIr.operation(
                "i64.extend_i32_s",
                watIr.rawWatNode(wat, WatNodeType.I32, "unconverted: inter-contract call"),
            );
        context.programAnalysis.warn(
            `unsupported inter-contract call to '${expression.callArguments[0]?.kind === AstKind.IDENTIFIER ? expression.callArguments[0].name : "?"}' (no callee IDL)`,
            expression.span.line,
        );
        return watIr.i64Constant(0);
    }
    const ai = context.lowering.emitAssetIter(context, expression, ContainerEmissionMode.VALUE);
    if (ai !== null) return watIr.rawWatNode(ai, WatNodeType.I64, "unconverted: asset iterator");
    const tc = context.lowering.emitThisCall(context, expression, true);
    if (tc !== null) return watIr.rawWatNode(tc, WatNodeType.I64, "unconverted: this-call");
    ...
    if (containerCallText !== null)
        return watIr.rawWatNode(
            containerCallText,
            WatNodeType.I64,
            "source-compiled instance method",
        );
    ...
    context.programAnalysis.warn(
        `unsupported call as value [${describeShape(expression)}]`,
        expression.span.line,
    );
    return watIr.i64Constant(0);
}
export function emitCallValue(
    context: FunctionEmissionContext,
    expression: Expression & {
        kind: AstKind.CALL;
    },
