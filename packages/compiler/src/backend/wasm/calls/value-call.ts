...
export function emitCallValueIr(
    ...
    if (expression.callee.kind === AstKind.IDENTIFIER && expression.callee.name === "__builtin_offsetof" && expression.callArguments.length === 2) {
    ...
    if (primitive?.kind === PlatformPrimitiveKind.MULTIPLY_HIGH) {
        ...
        if (output?.kind === AstKind.IDENTIFIER && context.localVars.get(output.name)?.wasmType === WatNodeType.I64) {
    ...
    if (context.proxyClass) {
        ...
        if (sib !== null) return watIr.rawWatNode(sib, WatNodeType.I64, "unconverted: proxy sibling call");
    }
    {
        const wrapperMethod = qpiWrapperMethod(expression);
        if (wrapperMethod) {
            const real = context.lowering.emitProposalProxyCall(context, expression, true);
            if (real !== null) return watIr.rawWatNode(real, WatNodeType.I64, "unconverted: proposal proxy call");
    ...
    if (expression.callee.kind === AstKind.IDENTIFIER && (expression.callee.name === "__qpi_call_other" || expression.callee.name === "__qpi_invoke_other")) {
        ...
        if (wat) return watIr.operation("i64.extend_i32_s", watIr.rawWatNode(wat, WatNodeType.I32, "unconverted: inter-contract call"));
    ...
    if (helperCallText !== null) return watIr.rawWatNode(helperCallText, WatNodeType.I64, "unconverted: helper call");
