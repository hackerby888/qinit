...
export function limbLayout(elemSize: number, count: number): StructLayout {
    let typeName = "uint8";
    if (elemSize === 8) typeName = "uint64";
    else if (elemSize === 4) typeName = "uint32";
    else if (elemSize === 2) typeName = "uint16";
    const type: TypeSpec = {
        kind: AstKind.NAME,
        name: typeName,
    };
...
export function resolveExpressionAddress(context: FunctionEmissionContext, expression: Expression): ResolvedAddress | null {
    ...
    if (expression.kind === AstKind.IDENTIFIER) {
        ...
        if (expression.name === "input")
            return {
                addr: "(local.get $__qinit_in)",
                type: null,
                size: context.in.size,
                layout: context.in,
            };
        if (expression.name === "output")
            return {
                addr: "(local.get $__qinit_out)",
                type: null,
                size: context.out.size,
                layout: context.out,
            };
