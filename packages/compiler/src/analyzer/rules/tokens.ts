...
export function findEntryFunctions(tokens: Token[]): EntryFunction[] {
    ...
    for (let index = 0; index < tokens.length; index++) {
        ...
        const closeParen = matchingToken(tokens, index + 1, TokenKind.L_PAREN, TokenKind.R_PAREN);
        if (closeParen < 0) {
            continue;
        }
        ...
        const bodyOpen = findNext(tokens, closeParen + 1, TokenKind.L_BRACE, TokenKind.SEMICOLON);
        if (bodyOpen < 0 || tokens[bodyOpen].kind !== TokenKind.L_BRACE) {
            continue;
        }
        const bodyClose = matchingToken(tokens, bodyOpen, TokenKind.L_BRACE, TokenKind.R_BRACE);
        if (bodyClose < 0) {
            continue;
        }

        const name = named ? nameToken.text : lifecycle;
        const plainMacro = named ? `${named[1]}_${named[2]}` : lifecycle;
        entries.push({
            name,
            ...
            withForm: named ? `${plainMacro}_WITH_LOCALS(${name})` : `${lifecycle}_WITH_LOCALS()`,
        });
        index = bodyClose;
    }
...
export function findLocalDeclarations(tokens: Token[], entry: EntryFunction): LocalDeclaration[] {
    ...
    for (let index = entry.bodyOpen + 1; index < entry.bodyClose; index++) {
        ...
        const declaration = parseLocalDeclaration(tokens, index, entry.bodyClose, forInitializer);
        if (!declaration) {
            continue;
        }
