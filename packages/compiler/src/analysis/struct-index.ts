...
export function registerCalleeContractDeclarations(
    ...
    for (const declaration of declarations) {
        if (declaration.kind === AstKind.VARIABLE) {
            ...
            if (
                !structDeclaration.bases?.some(
                    (baseType) =>
                        baseType.kind === AstKind.NAME && baseType.name === "ContractBase",
                )
            )
                continue;
...
export function walkNestedSegments(
    ...
    for (let segmentIndex = 0; segmentIndex < segs.length; segmentIndex++) {
        ...
        const ms = sd.members.find(
            (member): member is StructDecl =>
                member.kind === AstKind.STRUCT && member.name === seg && member.hasBody !== false,
        );
        if (ms) {
        ...
        const mt = sd.members.find(
            (member) => member.kind === AstKind.TYPEDEF_DECL && (member as any).name === seg,
        ) as any;
        if (!mt) return null;
