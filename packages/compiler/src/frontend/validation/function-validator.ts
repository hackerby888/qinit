...
export function checkRecursion(validator: Validator, _structDeclaration: StructDecl, fnBodies: Map<string, FunctionDecl>): void {
    ...
    const visit = (name: string, path: string[]): void => {
        ...
        if (st === ValidationVisitState.VISITING) {
            ...
            validator.error(`recursion is not allowed in a contract: ${cycle}`, fnBodies.get(name)?.span);
            return;
        }
...
export function checkFunctionBody(validator: Validator, fn: FunctionDecl, memberFns: Map<string, FnSig>): void {
    validator.currentFn = fn;
    validator.loopDepth = 0;
    validator.currentMemberFns = memberFns;
    validator.currentTypes = new Map(fn.params.map((parameter) => [parameter.name, parameter.type]));
...
export function checkReturns(validator: Validator, fn: FunctionDecl): void {
    ...
    validator.walkStatements(fn.body!, (statement) => {
        ...
        if (statement.value) {
            ...
            if (validator.isAggregateType(fn.returnType) && actual && !validator.isAggregateType(actual)) {
                validator.error(`return type is incompatible: cannot convert scalar expression to aggregate '${typeKey(fn.returnType)}'`, statement.span);
            } else if (
                actual &&
                ...
                validator.error(`return type mismatch: cannot convert '${typeKey(actual)}' to '${typeKey(fn.returnType)}'`, statement.span);
            }
        }
    });
    if (!isVoid && valueReturns === 0) {
        ...
        validator.error(`non-void function '${fn.name}' has a reachable fallthrough path without a return value`, fn.span);
    }
}
...
export function guaranteesReturn(validator: Validator, statement: Statement): boolean {
    ...
    if (statement.kind === AstKind.IF) return !!statement.else_ && validator.guaranteesReturn(statement.then) && validator.guaranteesReturn(statement.else_);
    if (statement.kind === AstKind.SWITCH) {
        ...
        const breaksOut = (statement: Statement): boolean => {
            ...
            if (statement.kind === AstKind.IF) return breaksOut(statement.then) || (!!statement.else_ && breaksOut(statement.else_));
            return false;
        };
        const last = body[body.length - 1];
        return body.some((bodyItem) => bodyItem.kind === AstKind.DEFAULT) && !body.some(breaksOut) && !!last && validator.guaranteesReturn(last);
    }
    return false;
}
