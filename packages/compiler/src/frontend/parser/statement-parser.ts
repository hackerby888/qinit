...
export class StatementParser {
                ...
                if (this.parser.state.pendingDeclarations.length) {
                    ...
                    while (this.parser.state.pendingDeclarations.length) {
                        ...
                        statements.push({
                            ...
                            span: (declaration as any).span ?? this.parser.state.peek().span,
                        });
                    }
                    return {
    ...
    parseFor(): Statement {
            ...
            if (isTypeKeyword(this.parser.state.peek().kind) || this.parser.expressions.looksLikeLocalDecl()) {
                ...
                if (declaration) {
                    initializer = {
                        kind: AstKind.DECLARATION,
                        declaration,
                        span: declaration.span ?? this.parser.state.peek().span,
                    };
                }
            } else {
        ...
        if (this.parser.state.peek().kind === TokenKind.SEMICOLON) this.parser.state.next();
        if (this.parser.state.peek().kind !== TokenKind.SEMICOLON) {
