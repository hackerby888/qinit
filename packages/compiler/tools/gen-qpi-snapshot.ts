...
const outputMatches = (path: string, contents: string) =>
    existsSync(path) && normalize(readFileSync(path, "utf8")) === normalize(contents);
...
function verify(hash: string): void {
    if (manifest.generatorVersion !== GENERATOR_VERSION) {
        throw new Error(
            `generator v${GENERATOR_VERSION} != manifest v${manifest.generatorVersion} — update core-snapshot.json`,
        );
    }
...
try {
    ...
    if (flag("--check")) {
        ...
        if (!outputMatches(protocolPreludePath, generated.protocolPreludeModule)) {
            throw new Error(
                `${protocolPreludePath} is stale; regenerate it from the pinned core checkout`,
            );
        }
        log(`${outputPath} is current`);
    } else {
        const snapshotChanged = !outputMatches(outputPath, generated.module);
        const protocolPreludeChanged = !outputMatches(
            protocolPreludePath,
            generated.protocolPreludeModule,
        );
        ...
        log(
            snapshotChanged || protocolPreludeChanged
                ? `generated core snapshot artifacts (${generated.hash})`
                : `up to date (${generated.hash})`,
        );
    }
} catch (error) {
