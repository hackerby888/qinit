...
async function checkSeed(contract: FuzzContract, headers: string, wasiAvailable: boolean, options: FuzzRunnerOptions): Promise<Finding | null> {
    ...
    const directory = mkdtempSync(join(tmpdir(), `${options.tempPrefix}-${contract.seed}-`));
...
export async function runFuzzer(options: FuzzRunnerOptions): Promise<void> {
    const { count, startSeed, jobs, oursOnly } = parseArguments(process.argv.slice(2));
    ...
    async function worker(): Promise<void> {
        while (cursor < seeds.length) {
            ...
            const finding = await checkSeed(contract, headers, wasiAvailable, options);
    ...
    for (const finding of findings) {
        findingsByKind.set(finding.kind, (findingsByKind.get(finding.kind) ?? 0) + 1);
    }
