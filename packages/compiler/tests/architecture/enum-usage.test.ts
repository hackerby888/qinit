...
const CONFIG_PATH = fileURLToPath(new URL("../../tsconfig.json", import.meta.url));
...
function isPropertyKeyLiteral(node: ts.LiteralTypeNode): boolean {
    for (let current = node.parent; current; current = current.parent) {
        ...
        if (ts.isIndexedAccessTypeNode(current) && containsNode(current.indexType, node)) {
            return true;
        }
...
describe("compiler enum usage", () => {
    test("keeps finite domains and comparisons enum-backed", () => {
        ...
        for (const source of program.getSourceFiles()) {
            if (source.isDeclarationFile || !source.fileName.startsWith(SOURCE_ROOT)) {
                continue;
            }
                ...
                if (
                    ...
                    for (const element of node.expression.expression.elements) {
                        if (ts.isStringLiteral(element)) {
                            addViolation(element, `searches enum with ${element.getText(source)}`);
                        }
                    }
                }
