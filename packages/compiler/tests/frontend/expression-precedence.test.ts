...
// A half-consumed expression reads as a shorter, passing one, so a parse is only accepted at EOF with no errors — the guard parseTypeFormat keeps in abi/type-format.
