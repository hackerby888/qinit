...
describe("semantic validation - control-flow graph", () => {
    const rejected = [
        ["rejects a non-void function with only one returning if branch", "uint64 f(bool condition) { if (condition) return 1; }"],
        ["rejects a non-void switch without a default return", "uint64 f(uint64 x) { switch (x) { case 0: return 1; case 1: return 2; } }"],
        ["rejects a loop that can terminate without returning", "uint64 f(bool condition) { while (condition) { return 1; } }"],
        ["rejects duplicate default labels", "uint64 f(uint64 x) { switch (x) { default: return 1; default: return 2; } }"],
        ["rejects a non-constant case label", "uint64 f(uint64 x) { switch (x) { case x: return 1; default: return 2; } }"],
        ...
        ["rejects goto that crosses an initialized local", "uint64 f() { goto done; uint64 value = 7; done: return value; }"],
        ...
        ["rejects binding a mutable reference to a temporary", "uint64 f() { uint64& value = 1; return value; }"],
        ["rejects a non-default parameter after a default parameter", "uint64 f(uint64 first = 1, uint64 second) { return first + second; }"],
        ["requires an explicit return even when a loop is syntactically infinite", "uint64 f() { while (true) {} }"],
    ] as const;
    ...
    const accepted = [
        ["accepts exhaustive if and else returns", "uint64 f(bool condition) { if (condition) return 1; else return 2; }"],
        ["accepts nested exhaustive branches", "uint64 f(bool a, bool b) { if (a) { if (b) return 1; else return 2; } else { return 3; } }"],
        ["accepts an exhaustive switch with a default", "uint64 f(uint64 x) { switch (x) { case 0: return 1; default: return 2; } }"],
        [
