...
describe("emit format parity", () => {
    ...
    test("call: zero-arg has no trailing space", () => {
        ...
        expect(emit(call("$copyMem", p, getL("q", WatNodeType.I32), i32c(16)))).toBe("(call $copyMem (local.get $p) (local.get $q) (i32.const 16))");
    });

    test("raw is verbatim", () => {
        expect(emit(raw("(i64.extend_i32_u (if (result i32) X (then (i32.const 1)) (else Y)))", WatNodeType.I64, "short-circuit"))).toBe(
...
describe("leaf-helper shapes (loadAt/storeAt/addrOf parity)", () => {
    ...
    test("narrowCast shapes", () => {
        ...
        expect(emit(operator("i64.extend_i32_u", operator("i64.ne", i64c(0), v)))).toBe("(i64.extend_i32_u (i64.ne (i64.const 0) (local.get $v)))");
    });

    test("compiler target primitive call shapes", () => {
        expect(emit(call("$intr_mulhi_u", v, i64c(3)))).toBe("(call $intr_mulhi_u (local.get $v) (i64.const 3))");
        expect(emit(call("$intr_mulhi_s", v, i64c(-3)))).toBe("(call $intr_mulhi_s (local.get $v) (i64.const -3))");
    });
});
...
describe("type assertions catch the silent-divergence class", () => {
    ...
    test("callSig covers dynamic targets", () => {
        ...
        expect(() => callSig({ params: [WatNodeType.I32], res: WatNodeType.VOID }, "$fn_x", v)).toThrow(/expected i32, got i64/);
    });
...
describe("CALL_SIG agrees with framework.ts", () => {
    test("handwritten QPI algorithm kernels cannot return", () => {
        ...
        const hits = files.flatMap((file) => [...readFileSync(file, "utf8").matchAll(forbidden)].map((match) => `${file}:${match[0]}`));
        expect(hits).toEqual([]);
    });
