...
describe("emit format parity", () => {
    ...
    test("generic op", () => {
        expect(emit(operator("i64.add", v, i64c(1)))).toBe(
...
describe("leaf-helper shapes (loadAt/storeAt/addrOf parity)", () => {
    test("loadScalar mirrors loadAt for every width and signedness", () => {
        ...
        expect(emit(loadScalar(p, 2, true))).toBe(
            "(i64.extend_i32_s (i32.load16_s (local.get $p)))",
        );
        expect(emit(loadScalar(p, 1))).toBe("(i64.extend_i32_u (i32.load8_u (local.get $p)))");
        expect(emit(loadScalar(p, 1, true))).toBe(
            "(i64.extend_i32_s (i32.load8_s (local.get $p)))",
        );
    ...
    test("narrowCast shapes", () => {
        expect(emit(operator("i64.and", v, i64c("0xff")))).toBe(
            "(i64.and (local.get $v) (i64.const 0xff))",
        );
...
describe("type assertions catch the silent-divergence class", () => {
    test("wrong operand width throws with the offending WAT", () => {
        expect(() => operator("i64.add", v, i32c(1))).toThrow(
            /expected i64, got i32.*\(i32\.const 1\)/,
        );
    ...
    test("void call in value position throws", () => {
        ...
        expect(() => assertTy(c, WatExpectedType.VALUE, "assignment RHS")).toThrow(
            /expected val, got void/,
        );
    ...
    test("callSig covers dynamic targets", () => {
        const helper = callSig(
            { params: [WatNodeType.I32, WatNodeType.I64], res: WatNodeType.I64 },
            "$lib0_probe",
            p,
            v,
        );
        ...
        expect(() =>
            callSig({ params: [WatNodeType.I32], res: WatNodeType.VOID }, "$fn_x", v),
        ).toThrow(/expected i32, got i64/);
    });
    ...
    test("drop accepts either value type, rejects void", () => {
        ...
        expect(() => operator("drop", call("$copyMem", p, p, i32c(1)))).toThrow(
            /expected val, got void/,
        );
    });
...
describe("CALL_SIG agrees with framework.ts", () => {
    test("handwritten QPI algorithm kernels cannot return", () => {
        ...
        const hits = files.flatMap((file) =>
            [...readFileSync(file, "utf8").matchAll(forbidden)].map(
                (match) => `${file}:${match[0]}`,
            ),
        );
        expect(hits).toEqual([]);
    ...
    test("every registry entry matches the framework definition", () => {
        ...
        for (const m of framework.matchAll(re)) {
            const params: string[] = [];
            for (const pm of m[2].matchAll(
                /\(param(?:\s+\$[a-zA-Z0-9_.]+)?((?:\s+(?:i32|i64))+)\)/g,
            )) {
