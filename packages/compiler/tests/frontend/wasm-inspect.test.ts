...
describe("Wasm module inspection", () => {
    test("accepts and describes the production generated ABI", async () => {
        ...
        expect(wat).toContain("(call $lh_acquireScratch (i64.extend_i32_u (local.get $size)) (i32.const 1))");
        ...
        expect(result.exports.find((exported) => exported.name === "dispatch")?.signature).toEqual(WASM_MODULE_EXPORT_ABI.dispatch);
        expect(result.exports.find((exported) => exported.name === "contract_index")?.signature).toEqual(WASM_MODULE_EXPORT_ABI.contract_index);

        const module = new WebAssembly.Module((await assemble(emitModule(SPEC))) as BufferSource);
        const lhost = Object.fromEntries(Object.entries(LHOST_ABI).map(([name, signature]) => [name, () => (signature.results[0] === "i64" ? 0n : 0)]));
    ...
    test("rejects the legacy arena_top export", async () => {
        const wat = addDefinition(emitModule(SPEC), '  (global (export "arena_top") (mut i32) (i32.const 0))');
        const result = inspectWasmModule(await assemble(wat));
  ...
  (import "lhost" "epoch" (func (param i64)))`,
        ...
        expect(result.ok).toBe(false);
        expect(
            result.diagnostics.some((diagnostic) => diagnostic.code === "unknown-import" && diagnostic.message.includes("wasi_snapshot_preview1.fd_write")),
        ).toBe(true);
        expect(result.diagnostics.some((diagnostic) => diagnostic.code === "unknown-import" && diagnostic.message.includes("notInCoreTable"))).toBe(true);
        expect(result.diagnostics.some((diagnostic) => diagnostic.code === "import-signature" && diagnostic.message.includes("lhost.epoch"))).toBe(true);
    });
    ...
    test("rejects missing and incorrectly typed ABI exports", async () => {
        ...
        expect(wrongContract.diagnostics.some((diagnostic) => diagnostic.code === "export-signature" && diagnostic.message.includes("contract_index"))).toBe(
            true,
        );
        ...
        expect(missing.diagnostics.some((diagnostic) => diagnostic.code === "missing-export" && diagnostic.message.includes("dispatch"))).toBe(true);

        const wrongStateAddr = emitModule(SPEC).replace("  (func $state_addr (result i32) (i32.const 0))", "  (func $state_addr (result i64) (i64.const 0))");
        const wrong = inspectWasmModule(await assemble(wrongStateAddr));
        expect(wrong.diagnostics.some((diagnostic) => diagnostic.code === "export-signature" && diagnostic.message.includes("state_addr"))).toBe(true);
    });
