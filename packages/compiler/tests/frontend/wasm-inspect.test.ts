...
describe("Wasm module inspection", () => {
    test("accepts and describes the production generated ABI", async () => {
        ...
        expect(
            lhostImports.find((imported) => imported.name === "acquireScratch")?.signature,
        ).toEqual({
  ...
  (import "lhost" "epoch" (func (param i64)))`,
        ...
        expect(
            result.diagnostics.some(
                (diagnostic) =>
                    diagnostic.code === "unknown-import" &&
                    diagnostic.message.includes("notInCoreTable"),
            ),
        ).toBe(true);
        expect(
            result.diagnostics.some(
                (diagnostic) =>
                    diagnostic.code === "import-signature" &&
                    diagnostic.message.includes("lhost.epoch"),
            ),
        ).toBe(true);
    });
    ...
    test("rejects missing and incorrectly typed ABI exports", async () => {
        ...
        expect(
            wrong.diagnostics.some(
                (diagnostic) =>
                    diagnostic.code === "export-signature" &&
                    diagnostic.message.includes("state_addr"),
            ),
        ).toBe(true);
    });
    ...
    for (const [label, field] of unsupported) {
        test(`rejects ${label}`, async () => {
            const result = inspectWasmModule(
                await assemble(addDefinition(emitModule(SPEC), field)),
            );
