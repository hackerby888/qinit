...
describe("edge audit — state.get read-only aliasing", () => {
    ...
    test("a function cannot mutate state.get through a non-const reference", async () => {
        await expectReadonlyRejection(wrap(`uint64 result;`, `PUBLIC_FUNCTION(Go) { bump(state.get().result); }`, `REGISTER_USER_FUNCTION(Go, 1);`));
    });

    test("read-only protection follows nested aggregate members", async () => {
        await expectReadonlyRejection(wrap(`Pair pair;`, `PUBLIC_PROCEDURE(Go) { bump(state.get().pair.left); }`, `REGISTER_USER_PROCEDURE(Go, 1);`));
    });
});
