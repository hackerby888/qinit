...
export function wamrToolchain(corePath: string): ToolchainStatus {
    ...
    // build-wasm is where core-lite's own wasm test target lands by default; it was missing here while cross-host did look for it, so the matrix said no WAMR.
    const candidates = [
        configured,
    ...
    return path
        ...
        : {
              available: false,
              detail: `WAMR qubic_wasm_tests not found (${candidates.join(", ")})`,
          };
}

export function toolchainTest(name: string, status: ToolchainStatus, body: () => void | Promise<void>, timeout = 180_000): void {
