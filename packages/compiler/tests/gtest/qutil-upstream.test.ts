    ...
    toolchainTest(
        ...
        async () => {
            ...
            if (compiler !== "typescript" && compiler !== "clang") {
                throw new Error(`GTEST_COMPILER must be "typescript" or "clang", got "${compiler}"`);
            }
            const runner = await buildRunner(CORE);
            const contracts = compiler === "clang" ? await buildContractsWithClang(CORE) : await buildContractsWithTypeScript(CORE);
