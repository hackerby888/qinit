...
function validateSource(source: string) {
  ...
  const parseErrors = parser.getDiagnostics().filter((diagnostic) => diagnostic.severity === "error");
...
describe("semantic validation - control-flow graph", () => {
  const rejected = [
    ...
    [
      "rejects a case label outside a switch",
      "uint64 f() { case 1: return 1; }",
    ],
    [
      "rejects break outside a loop or switch",
      "void f() { break; }",
    ],
    [
      "rejects continue outside a loop",
      "void f() { continue; }",
    ],
    [
      "rejects an undefined goto label",
      "void f() { goto missing; }",
    ],
    [
      "rejects duplicate goto labels",
      "void f() { again: ; again: ; }",
    ],
    [
    ...
    [
      "rejects assignment to an rvalue",
      "void f(uint64 value) { 1 = value; }",
    ],
    [
      "rejects incrementing a call result",
      "uint64 value() { return 1; } void f() { ++value(); }",
    ],
    [
  ...
  const accepted = [
    ...
    [
      "accepts a resolved forward goto",
      "uint64 f() { goto done; return 1; done: return 2; }",
    ],
  ] as const;
