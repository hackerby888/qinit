...
const emit = (line: string) => {
  console.log(line);
  if (BOARD) appendFileSync(BOARD, line + "\n");
};
const ARENA = 8 * 1024 * 1024;
const SCRATCH =
  "/tmp/claude-1000/-home-kali-Projects-core-lite/87fbc69c-6675-41cb-94cb-1c730c00fab8/scratchpad/dual";
...
async function nativeWasms(
  ...
  const build = async (cc: any, arenaSz: number): Promise<Uint8Array | null> => {
    const common = {
      contractPath: join(CORE, "src", "contracts", cc.file),
      name: cc.name,
      stateType: cc.stateType,
      slot: cc.index,
      corePath: CORE,
    };
    const p1 = await buildContract({
      ...common,
      outDir: join(SCRATCH, "n_" + cc.name),
      ...(shared ? { arenaSz } : {}),
    });
    if (!p1.so) {
      if (cc === c)
    ...
    const p2 = await buildContract({
      ...common,
      outDir: join(SCRATCH, "ns_" + cc.name),
      arenaSz,
      sharedMemBase: base,
    });
...
async function oursWasms(
  ...
  for (const d of deps) {
    ...
    const dopts = {
      ...
      slot: d.index,
      qpiHeader: headers,
      callees: cs.length ? cs : undefined,
      calleeSources: css.length ? css : undefined,
    };
...
for (const c of catalog) {
  ...
  let src: string;
  try {
    src = readFileSync(corpusPath, "utf8");
  } catch {
    continue;
  }
  ...
  try {
    const runner = await buildCorpusRunner({
      corpusPath,
      contractPath: join(CORE, "src", "contracts", c.file),
      name: c.name,
      stateType: c.stateType,
      slot: c.index,
      corePath: CORE,
      outDir: join(SCRATCH, "run_" + c.name),
      arenaSz: ARENA,
    });
    if (!runner.ok || !runner.so) {
      emit(
        `  HARNESS-BUILD  ${c.name.padEnd(11)} ${
          (runner.stderr ?? "")
            .split("\n")
    ...
    const tag =
      oursBug === 0 && harness === 0
        ...
        : oursBug > 0
          ...
          : "harness-gap ";
    emit(
      `  ${tag} ${c.name.padEnd(11)} green ${green}/${our.length}  ours-bug ${oursBug}  harness ${harness}${firstBug ? "  | " + firstBug : ""}`,
    );
  } catch (e: any) {
    console.log(`  THROW        ${c.name.padEnd(11)} ${String(e.message).slice(0, 64)}`);
  }
}
