...
const exprVal = (value: number): TypeSpec =>
  ({
    kind: "expr_value",
    expr: { kind: "int_literal", value: String(value), span: NO_SPAN },
  }) as TypeSpec;
...
describe("Codegen — template layout", () => {
  test("Array<uint64, 4> via layoutOfType", () => {
    ...
    cg.templates.set(
      ...
      ctmpl(
        [tparam("T"), ntparam("L")],
        [fld("_data", { kind: "array", elem: n("T"), size: ident("L") } as TypeSpec)],
      ),
    );
  ...
  test("missing Array template fails instead of using an approximate formula", () => {
    const cg = makeCg();
    expect(() => cg.sizeOfType(tinst("Array", [n("uint64"), exprVal(4)]))).toThrow(
  ...
  test("missing BitArray template fails instead of using an approximate formula", () => {
    const cg = makeCg();
    expect(() => cg.sizeOfType(tinst("BitArray", [exprVal(256)]))).toThrow(
      /not captured.*refusing an approximate layout/,
    );
  });
});
