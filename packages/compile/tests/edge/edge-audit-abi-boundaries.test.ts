...
describe("edge audit — inclusive QPI ABI boundaries", () => {
  beforeAll(async () => {
    ...
    entries = sim.contracts.get(27)!.entries;
  });

  test("a procedure input of exactly MAX_INPUT_SIZE bytes is registered", () => {
    expect(entries).toContainEqual({ it: 1, kind: 1, inSize: 1024, outSize: 0 });
  });

  test("a function output of exactly uint16 max bytes is registered", () => {
    expect(entries).toContainEqual({ it: 1, kind: 0, inSize: 0, outSize: 65535 });
  });

  test("locals of exactly MAX_SIZE_OF_CONTRACT_LOCALS compile", () => {
    expect(entries).toContainEqual({ it: 65535, kind: 1, inSize: 0, outSize: 0 });
  });
