...
function mutateEmbeddedAbi(header: string, mutate: (abi: ReturnType<typeof parseLiteAbiSource>) => void): string {
...
describe("source-backed ABI mutations", () => {
  test("a new scalar QPI wrapper compiles without a compiler method entry", async () => {
    const declared = addFunctionContextDeclaration(HEADER, "inline uint32 newScalar() const;")
    ...
    expect(WebAssembly.Module.imports(new WebAssembly.Module(moduleBytes))[0]).toMatchObject({ module: "lhost", name: "newScalar" });
  });

  test("a new aggregate-returning QPI wrapper uses the normal parsed layout path", async () => {
    const header = addFunctionContextDeclaration(HEADER, "inline id nextAlias(const id& value) const;")
  ...
  test("record field reorder changes generated offsets and capacity comes from core", async () => {
    const reordered = shared
      ...
      .replace("#define LITE_ASSET_ENTRY_CAPACITY 1024u", "#define LITE_ASSET_ENTRY_CAPACITY 2048u");
    ...
    `);
    const baseline = await compileContract({ source: iteratorContract, name: "AssetRecordBaseline", slot: 27, qpiHeader: HEADER, arenaSz: 1 << 20 });
    const changedHeader = mutateEmbeddedAbi(HEADER, (abi) => { abi.records.LiteAssetEntry = record; });
    const changed = await compileContract({ source: iteratorContract, name: "AssetRecordMutation", slot: 27, qpiHeader: changedHeader, arenaSz: 1 << 20 });
    expect(changed.diagnostics.filter((diagnostic) => diagnostic.severity === "error")).toEqual([]);
    expect(inspectLiteWasmModule(changed.wasm).memories[0].minimumPages)
      .toBeGreaterThan(inspectLiteWasmModule(baseline.wasm).memories[0].minimumPages);
  });

  test("system-procedure IDs and method names follow the canonical table", async () => {
    const changed = metadata
      .replace("X(INITIALIZE,               0, initialize,               __initializeEmpty)", "X(BEGIN_EPOCH,              0, beginEpoch,               __beginEpochEmpty)")
      .replace("X(BEGIN_EPOCH,              1, beginEpoch,               __beginEpochEmpty)", "X(INITIALIZE,               1, initialize,               __initializeEmpty)");
    ...
    const header = mutateEmbeddedAbi(HEADER, (abi) => {
      ...
      abi.systemProcedures.splice(0, 2,
        { ...beginEpoch, id: 0 },
        { ...initialize, id: 1 },
      );
    });
...
struct CONTRACT_STATE_TYPE : public ContractBase {
  ...
  REGISTER_USER_FUNCTIONS_AND_PROCEDURES() {}
    ...
    const result = await compileContract({ source, name: "SystemProcedureMutation", slot: 27, qpiHeader: header, arenaSz: 1 << 20 });
    expect(result.diagnostics.filter((diagnostic) => diagnostic.severity === "error")).toEqual([]);
  ...
  test("unsupported or ambiguous core metadata fails generation", () => {
    expect(() => parseLiteAbiSource(metadata.replace('GI("endFn"', 'GI("beginFn"'), shared)).toThrow(/duplicate LHOST import/);
    expect(() => parseLiteAbiSource(metadata, shared.replace("struct LiteAssetEntry {", "struct LiteAssetEntry {\n    float unsupported;"))).toThrow(/unsupported LiteAssetEntry field/);
    expect(() => parseLiteAbiSource(metadata.replace("X(BEGIN_EPOCH,              1", "X(BEGIN_EPOCH,              7"), shared)).toThrow(/ambiguous system-procedure order/);
  });
});
