...
for (const seed of process.argv.slice(2).map(Number)) {
  ...
  const ours = await compileContract({ source: c.source, contractName: `U${seed}`, slot: 27, qpiHeader: H, arenaSizeBytes: 1 << 20 });
  ...
  try {
    ...
    const built = await buildContractWithWasiClang({ contractPath: join(dir, "U.h"), name: "U", slot: 27, corePath: CORE, outDir: dir, skipVerify: true });
    if (!built.ok) {
