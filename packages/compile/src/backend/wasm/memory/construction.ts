...
export function resolveContainerElem(context: FunctionEmissionContext, expression: Expression & {
    ...
    if (!compiled || (compiled.cm.retKind !== "i32" && !compiled.cm.retAgg))
        return null;
...
export function emitConstruct(context: FunctionEmissionContext, dstAddr: string, type: TypeSpec, callArguments: Expression[]): boolean {
    ...
    context.lines.push(`    ${watIr.serializeWatNode(watIr.functionCall("$setMem", watIr.localGet(destinationBase, "i32"), watIr.i32Constant(layout.size), watIr.i32Constant(0)))}`);
