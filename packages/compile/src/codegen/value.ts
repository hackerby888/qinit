...
export function emitAssign(ctx: FnCtx, expr: Expression & { kind: "assign" }): string {
  ...
  if (lhs && expr.op === "=" && isUint128(ctx.cg, lhs.type)) {
    ctx.lines.push(
      `    ${ir.emit(ir.call("$copyMem", addrIr(lhs.addr), emitU128Ir(ctx, expr.right), ir.i32c(16)))}`,
    );
    return "";
  }
  ...
  if (lhs && expr.op === "=" && isAggregate(ctx, lhs.type, lhs.size)) {
    ...
    if (
      ...
      ((expr.right.kind === "call" &&
        expr.right.callee.kind === "identifier" &&
        /Asset(Ownership|Possession)Iterator$/.test(expr.right.callee.name)) ||
      ...
      if (arg) {
        ...
        ctx.lines.push(
          `    ${ir.emit(ir.storeRaw("i64.store", null, addrIr(lhs.addr), ir.i64c(0)))}`,
        ); // zero count+cursor
      }
      return "";
    }
    ...
    if (
      expr.right.kind === "construct" &&
      lhs.type &&
      emitConstruct(ctx, lhs.addr, lhs.type, expr.right.args)
    ) {
      return "";
    }
    // bare brace-init-list `target = { a, b, c };` — same field-wise materialization, typed by the target.
    if (
      expr.right.kind === "initializer_list" &&
      lhs.type &&
      emitConstruct(ctx, lhs.addr, lhs.type, expr.right.exprs)
    ) {
      return "";
    }
    ...
    if (src) {
      ctx.lines.push(
        `    ${ir.emit(ir.call("$copyMem", addrIr(lhs.addr), addrIr(src), ir.i32c(lhs.size)))}`,
      );
      return "";
    }
    ctx.cg.warn(
      `unsupported aggregate assignment [${describeShape(expr.left)} = ${describeShape(expr.right)}]`,
      expr.span.line,
    );
    return "";
  }
  ...
  if (lhs) {
    if (expr.op === "=") {
      ctx.lines.push(
        `    ${ir.emit(ir.storeScalar(addrIr(lhs.addr), lhs.size, emitValueIr(ctx, expr.right)))}`,
      );
      return "";
    }
    ...
    ctx.lines.push(
      `    ${ir.emit(ir.storeScalar(addrIr(lhs.addr), lhs.size, emitValueIr(ctx, compoundToBinary(expr))))}`,
    );
    return "";
  }
  ...
  if (expr.left.kind === "identifier" && isScalarLocal(ctx, expr.left.name)) {
    ...
    const rhs =
      expr.op === "=" ? emitValueIr(ctx, expr.right) : emitValueIr(ctx, compoundToBinary(expr));
...
export function emitValueIr(ctx: FnCtx, expr: Expression): ir.Ir {
  if (
    ...
    expr.member === "constructionEpoch" &&
    expr.object.kind === "subscript" &&
    expr.object.object.kind === "identifier" &&
    expr.object.object.name === "contractDescriptions"
  ) {
    return ir.op(
      "i64.extend_i32_u",
      ir.call("$qt_construction_epoch", ir.op("i32.wrap_i64", emitValueIr(ctx, expr.object.index))),
    );
  }
  if (
    expr.kind === "member_access" &&
  ...
  if (
    expr.kind === "member_access" &&
    (expr.member === "low" || expr.member === "high") &&
    isU128Expr(ctx, expr.object)
  ) {
  ...
  switch (expr.kind) {
    ...
    case "identifier": {
      ...
      if (p && p.isAddr && !ctx.cg.isAggregateType(p.type))
        return ir.loadScalar(
          ir.getL(p.local ?? expr.name, "i32"),
          ctx.cg.sizeOfType(p.type),
          !unsignedScalar(p.type),
        );
      if (expr.name === "SELF_INDEX")
        return ir.op("i64.extend_i32_u", ir.call("$qpi_contractIndex"));
      ...
      if (ctx.thisLayout) {
        ...
        if (tn && tn.size <= 8)
          return loadAtIr(tn.addr, tn.size, isSignedScalarType(tn.type, ctx.cg));
      }
    ...
    case "member_access": {
      ...
      if (ot?.kind === "inline_struct") {
        const sm = ot.struct.members.find(
          (m) =>
            m.kind === "variable" &&
            (m as VariableDecl).name === expr.member &&
            ((m as VariableDecl).isStatic || (m as VariableDecl).isConstexpr) &&
            (m as VariableDecl).init,
    ...
    case "template_call": {
      ...
      if (
        ...
        expr.callee.object.name === "qpi"
      ) {
        const source = emitTemplateContainerCall(ctx, expr, true);
        if (source !== null)
          return ir.raw(source, "i64", "source-compiled template instance method");
      }
      ctx.cg.warn(
        `unsupported template_call '${expr.callee.kind === "identifier" ? expr.callee.name : "?"}' as value`,
        expr.span.line,
      );
      return ir.i64c(0);
    }
    ...
    case "unary_op": {
      ...
      const canon32 = (n: ir.Ir) =>
        mask32
          ? ir.op("i64.and", n, ir.i64c("0xffffffff"))
          : sext32
            ? ir.op("i64.extend32_s", n)
            : n;
      switch (expr.op) {
        case "-": {
        ...
        case "!":
          return ir.op("i64.extend_i32_u", ir.op("i64.eqz", a));
        default:
          return a;
      }
    }
    ...
    case "ternary": {
      ...
      ctx.lines.push(
        `    (if (i64.ne (i64.const 0) ${ir.emit(cond)}) (then\n${thenB}\n    ) (else\n${elseB}\n    ))`,
      );
    ...
    case "static_cast":
      return narrowCastIr(
        emitValueIr(ctx, expr.expr),
        expr.type?.kind === "name" ? expr.type.name : undefined,
      );
    case "construct": {
    ...
    case "sizeof_expr": {
      ...
      if (expr.expr.kind === "identifier") {
        const sz = ctx.cg.sizeOfType(
          { kind: "name", name: expr.expr.name },
          ctx.thisBind ?? NO_BIND,
        );
  ...
  if (expr.kind === "call" && expr.callee.kind === "member_access") {
    ...
    if (ot?.kind === "template_instance") {
      ...
      if (mt?.def.returnType) {
        return isUint128(
          ctx.cg,
          ctx.cg.substInBindings(ctx.cg.derefType(mt.def.returnType), mt.bind),
        );
      }
    }
    ...
    const fn = struct?.members.find(
      (m) =>
        m.kind === "function" &&
        (m as FunctionDecl).name === (expr.callee as Expression & { kind: "member_access" }).member,
...
function sourceU128Result(
  ...
  if (compiled.cm.retKind === "i64")
    return ir.raw(compiled.call, "i64", "source-compiled uint128 scalar result");
  if (compiled.cm.retKind === "i32")
    return ir.raw(compiled.call, "i32", "source-compiled uint128 reference result");
  ctx.lines.push(`    ${compiled.call}`);
...
export function emitU128Ir(ctx: FnCtx, expr: Expression): ir.Ir {
  ...
  if (expr.kind === "construct" && isUint128(ctx.cg, expr.type))
    return constructU128(ctx, expr.args);
  if (expr.kind === "c_cast" || expr.kind === "static_cast") {
  ...
  if (expr.kind === "call" && expr.callee.kind === "identifier") {
    ...
    const constructor =
      expr.callee.name === "uint128" ||
      expr.callee.name === "uint128_t" ||
      (bound ? isUint128(ctx.cg, bound) : false);
    if (constructor) return constructU128(ctx, expr.args);

    if (
      symbolBaseName(expr.callee.name) === "div" &&
      ...
      return ir.raw(
        emitAggHelperCall(ctx, expr, helper),
        "i32",
  ...
  if (
    ...
    return ir.raw(
      emitAggHelperCall(ctx, callExpr, helper),
      "i32",
      "source-compiled uint128 div result",
    );
  }
  ...
  if (expr.kind === "ternary") {
    const destination = allocSlotIr(ctx, 16);
    ctx.lines.push(
      `    (if ${ir.emit(ir.op("i64.ne", ir.i64c(0), emitValueIr(ctx, expr.cond)))} (then`,
    );
    ctx.lines.push(
      `    ${ir.emit(ir.call("$copyMem", destination, emitU128Ir(ctx, expr.then), ir.i32c(16)))}`,
    );
    ctx.lines.push("    ) (else");
    ctx.lines.push(
      `    ${ir.emit(ir.call("$copyMem", destination, emitU128Ir(ctx, expr.else_), ir.i32c(16)))}`,
    );
    ctx.lines.push("    ))");
    return destination;
  }
  ...
  if (expr.kind === "binary_op") {
    ...
    if (expr.op === "|" || expr.op === "^") {
      ...
      ctx.lines.push(
        `    ${ir.emit(ir.storeRaw("i64.store", null, destination, ir.op(opcode, ir.loadRaw("i64.load", null, left), ir.loadRaw("i64.load", null, right))))}`,
      );
      ctx.lines.push(
        `    ${ir.emit(ir.storeRaw("i64.store", 8, destination, ir.op(opcode, ir.loadRaw("i64.load", 8, left), ir.loadRaw("i64.load", 8, right))))}`,
      );
      return destination;
    }

    const method = ["+", "-", "*", "/", "&", "<<", ">>"].includes(expr.op)
      ? `operator${expr.op}`
      : null;
    if (method) {
      const left = emitU128Ir(ctx, expr.left);
      ...
      const key =
        scalarRight && expr.op === "&"
          ...
          : scalarRight && expr.op === ">>"
            ...
            : "uint128_t";
      const right =
        key === "uint128_t" && scalarRight ? u128ConstructorExpr(expr.right) : expr.right;
...
export function unsignedScalar(t: TypeSpec | null | undefined): boolean {
  ...
  return (
    /^(uint|unsigned\b|size_t$|bool$|bit$)/.test(t.name) ||
    t.name === "uint128" ||
    t.name === "uint128_t"
  );
}
...
export function isUnsignedExpr(ctx: FnCtx, expr: Expression): boolean {
  switch (expr.kind) {
    case "c_cast":
    case "static_cast":
      return unsignedScalar(expr.type);
    case "paren":
      return isUnsignedExpr(ctx, expr.expr);
    case "int_literal":
      return /[uU]/.test(expr.suffix ?? "");
    case "identifier": {
      const p = ctx.params?.get(expr.name);
    ...
    case "call": {
      ...
      const owner =
        context?.kind === "name"
          ? separator >= 0
            ? context.name.slice(separator + 2)
            : context.name
          : context?.kind === "template_instance"
            ? context.name
            : null;
      if (!owner) return false;
      const method = ctx.cg.methodTemplate(
        owner,
        context?.kind === "template_instance" ? context.args : [],
        expr.callee.member,
        expr.args.length,
      );
    ...
    case "postfix_op":
      return isUnsignedExpr(ctx, expr.arg);
    case "ternary":
      return isUnsignedExpr(ctx, expr.then) || isUnsignedExpr(ctx, expr.else_);
    default:
      return false;
  }
}
...
export function scalarTypeInfo(
  ...
  switch (expr.kind) {
    case "paren":
      return scalarTypeInfo(ctx, expr.expr);
    case "c_cast":
    case "static_cast": {
    ...
    case "subscript": {
      const t =
        expr.kind === "identifier"
          ? (ctx.params?.get(expr.name)?.type ??
            ctx.refLocals?.get(expr.name) ??
            ctx.localVars.get(expr.name)?.type ??
            ctx.cg.typeOfConstant(expr.name) ??
            resolveAddr(ctx, expr)?.type ??
            null)
    ...
    case "prefix_op":
    case "postfix_op":
      return scalarTypeInfo(ctx, expr.arg);
    case "call":
    case "template_call": {
...
export function emitBinaryIr(ctx: FnCtx, expr: Expression & { kind: "binary_op" }): ir.Ir {
  ...
  if (expr.op === "==" || expr.op === "!=") {
    ...
    if (la && ra) {
      const eq = ir.call(
        "$memeq",
        ir.raw(la.addr, "i32", "lvalue address channel"),
        ir.raw(ra.addr, "i32", "lvalue address channel"),
        ir.i32c(Math.min(la.size, ra.size)),
      );
      return expr.op === "=="
        ? ir.op("i64.extend_i32_u", eq)
        : ir.op("i64.extend_i32_u", ir.op("i32.eqz", eq));
    }
  }
  ...
  if (expr.op === "<" || expr.op === ">" || expr.op === "<=" || expr.op === ">=") {
    ...
    if (la && ra && la.size === 32 && ra.size === 32) {
      const lt = (x: { addr: string }, y: { addr: string }) =>
        ir.call(
          "$m256_lt",
          ir.raw(x.addr, "i32", "lvalue address channel"),
          ir.raw(y.addr, "i32", "lvalue address channel"),
        );
  ...
  if (expr.op === "&&" || expr.op === "||") {
    ...
    if (rLines.length === 0) {
      return expr.op === "||"
        ? ir.raw(
            `(i64.extend_i32_u (if (result i32) ${ir.emit(lb)} (then (i32.const 1)) (else ${ir.emit(rb)})))`,
            "i64",
            "inline if-expression",
          )
        : ir.raw(
            `(i64.extend_i32_u (if (result i32) ${ir.emit(lb)} (then ${ir.emit(rb)}) (else (i32.const 0))))`,
            "i64",
            "inline if-expression",
          );
    }
    ...
    if (expr.op === "||") {
      ctx.lines.push(
        `    (if ${ir.emit(lb)} (then (local.set $${tmp} (i32.const 1))) (else\n${rBranch}\n    ))`,
      );
    } else {
      ctx.lines.push(
  ...
  const wrapL = (n: ir.Ir, active: boolean) =>
    active ? ir.op("i64.and", n, ir.i64c("0xffffffff")) : n;
  ...
  switch (expr.op) {
    case "+":
      return wrapS(wrapL(ir.op("i64.add", l, r), wrap32), swrap32);
    case "-":
      return wrapS(wrapL(ir.op("i64.sub", l, r), wrap32), swrap32);
    case "*":
      return wrapS(wrapL(ir.op("i64.mul", l, r), wrap32), swrap32);
    case "/":
      return ir.op(u ? "i64.div_u" : "i64.div_s", lc, rc);
    case "%":
      return ir.op(u ? "i64.rem_u" : "i64.rem_s", lc, rc);
    case "<<": {
    ...
    case ">>":
      return ir.op(li.unsigned ? "i64.shr_u" : "i64.shr_s", l, shiftCount(r));
    case "&":
      return ir.op("i64.and", l, r);
    case "|":
      return wrapL(ir.op("i64.or", l, r), wrap32);
    case "^":
      return wrapL(ir.op("i64.xor", l, r), wrap32);
    case "==":
      return cmp("i64.eq");
    case "!=":
      return cmp("i64.ne");
    case "<":
      return cmp(u ? "i64.lt_u" : "i64.lt_s");
    case ">":
      return cmp(u ? "i64.gt_u" : "i64.gt_s");
    case "<=":
      return cmp(u ? "i64.le_u" : "i64.le_s");
    case ">=":
      return cmp(u ? "i64.ge_u" : "i64.ge_s");
    default:
      return ir.i64c(0);
  }
}
