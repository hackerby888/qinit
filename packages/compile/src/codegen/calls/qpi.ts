...
function parsedAggregateLayout(ctx: FnCtx, name: string) {
  ...
  const firstField = (...fieldNames: string[]): number => {
    ...
    if (!fieldName) throw new Error(`core QPI aggregate '${name}' is missing fields '${fieldNames.join("' or '")}'`);
...
export function materializeSelect(ctx: FnCtx, expression: Expression | undefined): string {
  ...
  ctx.lines.push(`    ${ir.emit(ir.call("$setMem", slot, ir.i32c(parsed.layout.size), ir.i32c(0)))}`);
  const flag = (offset: number, value: number) => ctx.lines.push(`    ${ir.emit(ir.storeRaw("i32.store8", null, ir.addr0(slot, offset), ir.i32c(value)))}`);
  const clamp = (value: Expression, mnemonic: string, offset: number, mask: string) =>
    ctx.lines.push(`    ${ir.emit(ir.storeRaw(mnemonic, null, ir.addr0(slot, offset), ir.op("i32.and", ir.op("i32.wrap_i64", emitValueIr(ctx, value)), ir.i32c(mask))))}`);
  const callExpression = expression?.kind === "call" ? expression : undefined;
  const staticName = callExpression
    ? (callExpression.callee.kind === "qualified_name" ? callExpression.callee.name : callExpression.callee.kind === "member_access" ? callExpression.callee.member : null)
    : null;
  if (!expression || staticName === "any") {
    ...
    if (!callExpression?.args[0]) throw new Error("byManagingContract selector is missing its contract index");
    ...
    if (source) ctx.lines.push(`    ${ir.emit(ir.call("$copyMem", slot, addrIr(source), ir.i32c(32)))}`);
    else if (expression.exprs[0]) throw new Error("asset selector id is not addressable");
    if (expression.exprs[1]) clamp(expression.exprs[1], "i32.store16", parsed.field("managingContract"), "0xffff");
    if (expression.exprs[2]) clamp(expression.exprs[2], "i32.store8", parsed.firstField("anyOwner", "anyPossessor", "anyId"), "1");
    if (expression.exprs[3]) clamp(expression.exprs[3], "i32.store8", parsed.field("anyManagingContract"), "1");
  } else {
    const source = emitAddr(ctx, expression);
    if (!source) throw new Error("asset selector is not addressable");
    ctx.lines.push(`    ${ir.emit(ir.call("$copyMem", slot, addrIr(source), ir.i32c(parsed.layout.size)))}`);
  }
  return ir.emit(slot);
}

export function materializeAssetAddress(ctx: FnCtx, expression: Expression | undefined, bindingName: string): string {
  if (expression?.kind === "initializer_list") {
    ...
    ctx.lines.push(`    ${ir.emit(ir.storeRaw("i64.store", null, ir.addr0(slot, parsed.field("assetName")), emitValueIr(ctx, expression.exprs[1])))}`);
    return ir.emit(slot);
  }
  const address = expression ? (resolveAddr(ctx, expression)?.addr ?? emitAddr(ctx, expression)) : null;
...
export function emitQpiCall(ctx: FnCtx, expression: Expression & { kind: "call" }): null {
  if (!(expression.callee.kind === "member_access"
    ...
    && expression.callee.object.name === "qpi")) return null;
  const contextType = ctx.params?.get("qpi")?.type;
  if (contextType?.kind === "name" && ctx.cg.hasInstanceMethod(contextType.name, expression.callee.member)) return null;
  if (contextType?.kind === "name" && /QpiContextFunctionCall$/.test(contextType.name)
