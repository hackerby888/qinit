...
export function resolveProxyTarget(
  ...
  return {
    addr: node.addr,
    pvType: { kind: "template_instance", name: "ProposalVoting", args, span: pvt.span },
  };
}
...
export function emitProposalProxyAddr(
  ...
  if (!method) return null;
  const xExpr = (
    (expr.callee as Expression & { kind: "member_access" }).object as Expression & { kind: "call" }
  ).args[0];
  if (!xExpr) return null;
  const target = resolveProxyTarget(ctx, xExpr);
  if (!target) return null;
  ...
  ctx.lines.push(
    `    (call ${cm.label} ${s} ${target.addr} (i32.const 0)${ops.length ? " " + ops.join(" ") : ""})`,
  );
  return s;
}
...
export function emitProxySiblingCall(
  ...
  const known =
    ctx.cg.templateMethods.get(ctx.proxyClass)?.has(method) ||
    ctx.cg.templateMethods.get("QpiContextProposalFunctionCall")?.has(method);
...
export function callProxy(
  ...
  if (cm.retAgg) {
    const s = allocSlot(ctx, cm.retAgg);
    ctx.lines.push(
      `    (call ${cm.label} ${s} ${self} (i32.const 0)${ops.length ? " " + ops.join(" ") : ""})`,
    );
  ...
  ctx.lines.push(
    cm.retKind === "i64"
      ? `    ${ir.emit(ir.op("drop", ir.raw(call, "i64", "unconverted: proxy method call")))}`
      : `    ${call}`,
  );
  return "";
}
...
export function compileProxyMethod(
  ...
  const bind: Bindings = {
    types: new Map([
      ["ProposerAndVoterHandlingType", P],
      ["ProposalDataType", D],
    ]),
    values: new Map(),
    structs: new Map(),
  ...
  const cm: CompiledMethod = {
    label: `$PV${cg.compiledMethods.size}_${proxyClass}_${method}`,
    fnParams,
    retKind,
    retAgg,
    retType: retT,
...
export function emitProxyMethodFn(
  ...
  for (const p of cm.fnParams)
    ctx.params!.set(p.name, {
      wasmType: p.wasmType,
      isAddr: p.isAddr,
      type: cg.substInBindings(cg.derefType(p.type), bind),
    });
