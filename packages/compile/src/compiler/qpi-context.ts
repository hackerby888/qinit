...
function getQpiPrelude(headers: string): { text: string; macros: Map<string, MacroDef> } {
  ...
  const text = pp.preprocess({ source: "", qpiHeader: mainHeaders, contractName: "__lib__", contractIndex: 0 });
...
export function getQpiContext(headers: string): QpiContext {
    ...
    for (const [cls, methods] of implLib.templateMethods) {
      ...
      for (const [name, definition] of methods) {
        if (sourceBackedHostWrappers && cls.startsWith("QpiContext")) {
          ...
          const declared = lib.globalStructs.get(cls)?.members.find((member) =>
            member.kind === "function" && member.name === baseName
            && member.params.length === (definition.fnParams ?? []).length,
          );
          const merged = declared?.kind === "function"
            ? {
                ...definition,
        ...
        else if (!lib.templateMethods.get(cls)!.has(name)) lib.templateMethods.get(cls)!.set(name, definition);
      }
    }
  ...
  const orderedImports = new Map<string, (typeof lib.importedFunctions extends Map<string, infer V> ? V : never)>();
  ...
  const extraImports = [...lib.importedFunctions.keys()].filter((name) => !orderedImports.has(name));
