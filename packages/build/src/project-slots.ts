...
export function planProjectSlots(
  ...
  const deployedBySlot = new Map(
    deployed.map((contract) => [contract.index, contract]),
  );
  ...
  const unassigned = customNodes.filter(
    (node) => !assignments.has(node.stateType),
  );
