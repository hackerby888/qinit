// Scaffold a standard gtest (core-lite `contract_testing.h`) from a contract's IDL: a `ContractTesting<Name>` fixture with one TEST per procedure/function.
...
export function genStdGtest(idl: ContractIdl, name: string, stateType: string = name): string {
    ...
    const tests = [initTest, ...procedures.map(procTest), ...functions.map(funcTest)].join("\n\n");
