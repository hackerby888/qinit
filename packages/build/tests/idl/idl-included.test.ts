...
const idlOf = (c: string, withPrelude = false) =>
  extractIdl(
    readFileSync(`${CORE}/src/contracts/${c}.h`, "utf8"),
    c,
    withPrelude ? { prelude: prelude() } : undefined,
  );
...
const fmts = (idl: ReturnType<typeof extractIdl>) =>
  Object.values({ ...idl.functions, ...idl.procedures }).flatMap(
    (e) => [e.in, e.out].filter(Boolean) as string[],
  );
function unresolved(idl: ReturnType<typeof extractIdl>): string[] {
  const out = new Set<string>();
  for (const f of fmts(idl))
...
test.skipIf(!have("Quottery"))(
  "Quottery: a struct preceded by a `// gov struct` comment resolves (QtryGOV)",
  () => {
    const idl = idlOf("Quottery");
    const pv = Object.values({ ...idl.functions, ...idl.procedures }).find(
      (e) => e.name === "ProposalVote",
    )!;
...
test.skipIf(!haveCore)(
  "prelude resolves the ambient qpi proposal-voting types (ProposalDataYesNo et al.)",
  () => {
...
test.skipIf(!haveCore)(
  "prelude clears every Proposal* type across the governance contracts",
  () => {
    for (const c of [
...
test.skipIf(!haveCore)(
  "SetProposal input expands ProposalDataYesNo to a concrete struct layout",
  () => {
    const ccf = idlOf("ComputorControlledFund", true);
    const sp = Object.values({ ...ccf.functions, ...ccf.procedures }).find(
      (e) => e.name === "SetProposal",
    )!;
    ...
    expect(
      unresolved({ functions: {}, procedures: { 0: sp } } as any).filter((t) =>
...
test.skipIf(!have("QUtil"))(
  "prelude include-expansion resolves OI::Price::OracleQuery from the per-interface header",
  () => {
    ...
    expect(byName.find((e) => e.name === "QueryPriceOracle")!.in).toBe(
      "{ id, uint64, id, id }, uint32",
    );
...
test.skipIf(!haveCore)(
  "a missing prelude degrades gracefully (types stay unknown, no throw)",
  () => {
    ...
    const idl = extractIdl(
      readFileSync(`${CORE}/src/contracts/ComputorControlledFund.h`, "utf8"),
      "CCF",
      { prelude: empty },
    );
    expect(Object.keys(idl.functions).length + Object.keys(idl.procedures).length).toBeGreaterThan(
      0,
    );
  },
);
