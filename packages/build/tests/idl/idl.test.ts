...
import { AbiScalarKind, AbiTypeKind, QINIT_IDL_VERSION, extractIdl } from "../../src/compile/idl";
...
test("state uses typed array and container nodes", () => {
    expect(idl.state.fields.map((field) => field.name)).toEqual(["counter", "nums", "balances"]);
