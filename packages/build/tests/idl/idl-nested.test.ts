...
// Verify named nested-field trees, including arrays and parent-scoped types with the same name.
...
const fn = (idl: ReturnType<typeof extractIdl>, name: string) =>
  Object.values(idl.functions).find((e) => e.name === name)!;
const proc = (idl: ReturnType<typeof extractIdl>, name: string) =>
  Object.values(idl.procedures).find((e) => e.name === name)!;
const allFields = (idl: ReturnType<typeof extractIdl>): Field[] =>
  Object.values({ ...idl.functions, ...idl.procedures }).flatMap((e) => [
    ...(e.inFields ?? []),
    ...(e.outFields ?? []),
  ]);
...
test.skipIf(!have("Qx"))(
  "QX: same-name nested `Order` resolves per parent (scoped, not first-declared)",
  () => {
    const idl = idlOf("Qx");
...
test.skipIf(!have("Qx"))("QX: nested `Asset` input resolves to a named struct tree", () => {
  ...
  expect(asset.struct?.map((f) => [f.name, f.type])).toEqual([
    ["issuer", "id"],
    ["assetName", "uint64"],
  ]);
});
...
test.skipIf(!have("Quottery"))(
  "Quottery: CreateEvent nested input struct keeps its member names",
  () => {
...
test.skipIf(!have("Qearn"))(
  "QEARN: deep Array<struct> state parses to a nested tree (no crash)",
  () => {
    ...
    for (const f of arrayOfStruct)
      expect(f.struct!.every((c) => typeof c.name === "string" && c.name.length)).toBe(true);
  },
);
...
test.skipIf(!have("QUtil"))(
  "QUtil: multi-variable declarations expand in the format string (not just the tree)",
  () => {
