...
test.if(HAS_CORE)("stripped source compiles with no shim at all, and matches the neutered build byte for byte", async () => {
    ...
    const stripped = await compileContractWithTypeScript({ ...options, source: stripCheatcodes(SOURCE), cheats: "off" });
    ...
    const neutered = await compileContractWithTypeScript({ ...options, source: SOURCE, cheats: "noop" });
