...
test("imports only the runtime calls used by the contract", () => {
    ...
    const functionsOnly = generateClient({ ...base, procedures: [] }, 28, {
        runtimeImport: "./runtime",
    });
    const proceduresOnly = generateClient({ ...base, functions: [] }, 28, {
        runtimeImport: "./runtime",
    });
...
test("no-input function: no args param, single-output map", () => {
    ...
    has("1, { type: Get_function_input_schema, value: {} }, Get_function_output_schema)");
...
test("flat scalar input uses the embedded typed schema", () => {
    ...
    has("2, { type: Sum_function_input_schema, value: args }, Sum_function_output_schema)");
