...
Tier `full` · 449 archetypes · 6649 contracts · generator version 2.
...
Only 2914 of 6649 contracts are faithful ports; the rest are marked `shape-only` in
`manifest.jsonl`. QPI has no revert, no `uint256`, no unbounded mapping and no reentrancy, so a
Solidity test whose point was atomicity or 256-bit wraparound becomes a _different_ test after
